...
#define GPIO_REG(reg)  ((volatile uint32_t *)(GPIO + (reg << 2)))
#define UART_REG(reg) ((volatile uint32_t *)(UART0 + (reg << 2)))
...
/* UART1 Init*/
void uart_init() {
  if (rcc_read_reg(PLL_ENABLE) == 1) {
    /* 配置 PLL */
    // *RCC_REG(CTLR) |= (uint32_t)0x00000001;
    // *RCC_REG(CFGR0) |= (uint32_t)(0x00280000);

    /* 配置 PLL */
    while((*RCC_REG(LOCK) & (uint32_t)0x02000000) == 0);
