...
// wire [8*$clog2(ROW_NUM_2)-1:0] buffer_2bit_WADR;
// wire [8*$clog2(ROW_NUM_4)-1:0] buffer_4bit_WADR;

genvar i;
generate
    for(i = 0; i < 8; i = i + 1) begin
        assign valid_2bit[i] = (WADR_2bit[i*(1+$clog2(ROW_NUM_2)) +: (1+$clog2(ROW_NUM_2))] <= (ROW_NUM_2 - 1)) ? outlier_valid_2bit[i] : 1'b0;
        assign valid_4bit[i] = (WADR_2bit[i*(1+$clog2(ROW_NUM_2)) +: (1+$clog2(ROW_NUM_2))] <= (ROW_NUM_2 - 1)) ? outlier_valid_4bit[i] : (outlier_valid_2bit[i] | outlier_valid_4bit[i]);
        assign value_4bit[i*4 +: 4] =  (WADR_2bit[i*(1+$clog2(ROW_NUM_2)) +: (1+$clog2(ROW_NUM_2))] <= (ROW_NUM_2 - 1)) ? outlier_value_4bit[i*4 +: 4] : 
                                        outlier_valid_2bit[i] ? {{2{outlier_value_2bit[2*(i+1)-1]}} , outlier_value_2bit[i*2 +: 2]}: outlier_value_4bit[i*4 +: 4];
    end    
endgenerate
assign index_2bit = outlier_index_2bit;
assign index_4bit = outlier_index_2bit | outlier_index_4bit;
assign value_2bit = outlier_value_2bit;
...
outlier_process_large_Macro #(
    ...
    .WADR_2bit              ({WADR_2bit[46:42],WADR_2bit[40:36],WADR_2bit[34:30],WADR_2bit[28:24],WADR_2bit[22:18],WADR_2bit[16:12],WADR_2bit[10:6],WADR_2bit[4:0]}),
    .WADR_4bit              (WADR_4bit),
