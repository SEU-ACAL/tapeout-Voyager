...
//
//       Instance Name:              arm28hkcpdpsram256x64m4
//       Words:                      256
//       Bits:                       64
//       Mux:                        4
//       Drive:                      6
...
//       
//       Creation Date:  Tue Jul 15 11:54:08 2025
//       Version: 	r0p1
//
...
module datapath_latch_arm28hkcpdpsram256x64m4 (CLK,Q_update,D_update,SE,SI,D,DFTRAMBYP,mem_path,XQ,Q);
...
endmodule // datapath_latch_arm28hkcpdpsram256x64m4
...
`ifdef POWER_PINS
module arm28hkcpdpsram256x64m4 (VDDCE, VDDPE, VSSE, CENYA, WENYA, AYA, CENYB, WENYB,
    ...
    SIB, SEB, COLLDISN);
`else
module arm28hkcpdpsram256x64m4 (CENYA, WENYA, AYA, CENYB, WENYB, AYB, GWENYA, GWENYB,
...
`endif
  ...
  wire [63:0] DA_int_bmux;
  reg [63:0] mem_path_A;
  reg [63:0] partial_mask;
  reg [63:0] partial_mask_A;
  reg [63:0] partial_corrupt_A = 64'b0;
  wire [63:0] QB_int;
  reg XQB, QB_update;
  reg XDB_sh, DB_sh_update;
  wire [63:0] DB_int_bmux;
  reg [63:0] mem_path_B;
  reg [63:0] partial_mask_B;
  reg [63:0] partial_corrupt_B = 64'b0;
 ...
 wire  CLKB_;
  ...
  assign `ARM_UD_DP WENYA_ = (RET1N_ | pre_charge_st) ? ({64{DFTRAMBYP_}} & (TENA_ ? WENA_ : TWENA_)) : {64{1'bx}};
  ...
  assign `ARM_UD_DP WENYB_ = (RET1N_ | pre_charge_st) ? ({64{DFTRAMBYP_}} & (TENB_ ? WENB_ : TWENB_)) : {64{1'bx}};
  ...
  assign `ARM_UD_DP GWENYB_ = (RET1N_ | pre_charge_st) ? (DFTRAMBYP_ & (TENB_ ? GWENB_ : TGWENB_)) : 1'bx;
   `ifdef ARM_FAULT_MODELING
     arm28hkcpdpsram256x64m4_error_injection u1(.CLK(CLKA_), .Q_out(QA_), .A(AA_int), .CEN(CENA_int), .DFTRAMBYP(DFTRAMBYP_int), .SE(SEA_int), .GWEN(GWENA_int), .WEN(WENA_int), .Q_in(QA_int));
  `else
  assign `ARM_UD_SEQ QA_ = (RET1N_ | pre_charge_st) ? ((QA_int)) : {64{1'bx}};
  `endif
  assign `ARM_UD_SEQ QB_ = (RET1N_ | pre_charge_st) ? ((QB_int)) : {64{1'bx}};
...
`endif
      ...
      row = mem[row_address];
        ...
        mem_path_A = {shifted_readLatch0[63], shifted_readLatch0[62], shifted_readLatch0[61],
     ...
     if (GWENA_int !== 1)
      ...
      if( (isBitX(GWENA_int) && DFTRAMBYP_int!==1) || isBitX(DFTRAMBYP_int) ) begin
        ...
        DA_int = {64{1'bx}};
          ...
          writeEnable = ~ ( {64{GWENA_int}} | {WENA_int[63], WENA_int[62], WENA_int[61],
      ...
      if (GWENA_int !== 1'b1 || DFTRAMBYP_int === 1'b1 || DFTRAMBYP_int === 1'bx) begin
        row_mask =  ( {3'b000, writeEnable[63], 3'b000, writeEnable[62], 3'b000, writeEnable[61],
          ...
          3'b000, writeEnable[5], 3'b000, writeEnable[4], 3'b000, writeEnable[3], 3'b000, writeEnable[2],
          3'b000, writeEnable[1], 3'b000, writeEnable[0]} << mux_address);
        new_data =  ( {3'b000, DA_int[63], 3'b000, DA_int[62], 3'b000, DA_int[61],
...
`endif
    ...
    #0;
      if (((previous_CLKA == previous_CLKB)) && (CENA_int !== 1'b1 && CENB_int !== 1'b1 && DFTRAMBYP_ !== 1'b1) && COLLDISN_int === 1'b1 && row_contention(AA_int,
        AB_int, ({64{GWENA_int}}|WENA_int), ({64{GWENB_int}}|WENB_int))) begin
        ...
        if (GWENA_int !== 1'b1 && GWENB_int !== 1'b1) begin
	      if (is_contention(AA_int, AB_int, ({64{GWENA_int}}|WENA_int), ({64{GWENB_int}}|WENB_int))) begin
          ...
          DA_int = ({64{1'bx}} & partial_mask_A) | (~partial_mask_A & DA_int);
  	WENA_int =  (({64{1'bx}} & partial_mask_A) | WENA_int) ;
          readWriteA;
          DB_int = ({64{1'bx}} & partial_mask_A) | (~partial_mask_A & DB_int);
  	WENB_int =  (({64{1'bx}} & partial_mask_A) | WENB_int) ;
          readWriteB;
		...
		if (is_contention(AA_int, AB_int, ({64{GWENA_int}}|WENA_int), ({64{GWENB_int}}|WENB_int))) begin
         if((|WENA_int) == 1'b1) begin
          ...
          readWriteB;
          readWriteA;
          partial_mask = ~{WENA_int[63], WENA_int[62], WENA_int[61], WENA_int[60],
        ...
        mem_path_B = (partial_mask & {64{1'bx}}) | (~partial_mask & mem_path_B);
        #0;
        QB_update = 1'b0;
        #0;
        QB_update = 1'b1;
		...
		if (is_contention(AA_int, AB_int, ({64{GWENA_int}}|WENA_int), ({64{GWENB_int}}|WENB_int))) begin
         if((|WENB_int) == 1'b1) begin
          ...
          readWriteB;
          readWriteA;
          partial_mask = ~{WENB_int[63], WENB_int[62], WENB_int[61], WENB_int[60],
        ...
        mem_path_A = (partial_mask & {64{1'bx}}) | (~partial_mask & mem_path_A);
        #0;
        QA_update = 1'b0;
        #0;
        QA_update = 1'b1;
...
`endif
       ...
       === 1'bx)  && row_contention(AA_int, AB_int, ({64{GWENA_int}}|WENA_int), ({64{GWENB_int}}|WENB_int))) begin
          ROW_CC = 1;
          READ_READ_1 = 0;
          READ_WRITE_1 = 0;
          WRITE_WRITE_1 = 0;
        ...
        if (GWENB_int !== 1'b1 && (& WENB_int) !== 1'b1) begin
          ...
          readWriteB;
        XQB = 1'b1; QB_update = 1'b1;
        end else if (is_contention(AA_int, AB_int, ({64{GWENA_int}}|WENA_int), ({64{GWENB_int}}|WENB_int))) begin
...
`endif
  ...
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA0 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(SIA_int[0]), .D(DA_int_bmux[0]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[0]), .XQ(XQA|partial_corrupt_A[0]), .Q(QA_int[0]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA1 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[0]), .D(DA_int_bmux[1]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[1]), .XQ(XQA|partial_corrupt_A[1]), .Q(QA_int[1]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA2 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[1]), .D(DA_int_bmux[2]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[2]), .XQ(XQA|partial_corrupt_A[2]), .Q(QA_int[2]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA3 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[2]), .D(DA_int_bmux[3]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[3]), .XQ(XQA|partial_corrupt_A[3]), .Q(QA_int[3]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA4 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[3]), .D(DA_int_bmux[4]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[4]), .XQ(XQA|partial_corrupt_A[4]), .Q(QA_int[4]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA5 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[4]), .D(DA_int_bmux[5]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[5]), .XQ(XQA|partial_corrupt_A[5]), .Q(QA_int[5]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA6 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[5]), .D(DA_int_bmux[6]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[6]), .XQ(XQA|partial_corrupt_A[6]), .Q(QA_int[6]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA7 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[6]), .D(DA_int_bmux[7]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[7]), .XQ(XQA|partial_corrupt_A[7]), .Q(QA_int[7]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA8 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[7]), .D(DA_int_bmux[8]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[8]), .XQ(XQA|partial_corrupt_A[8]), .Q(QA_int[8]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA9 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[8]), .D(DA_int_bmux[9]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[9]), .XQ(XQA|partial_corrupt_A[9]), .Q(QA_int[9]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA10 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[9]), .D(DA_int_bmux[10]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[10]), .XQ(XQA|partial_corrupt_A[10]), .Q(QA_int[10]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA11 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[10]), .D(DA_int_bmux[11]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[11]), .XQ(XQA|partial_corrupt_A[11]), .Q(QA_int[11]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA12 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[11]), .D(DA_int_bmux[12]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[12]), .XQ(XQA|partial_corrupt_A[12]), .Q(QA_int[12]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA13 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[12]), .D(DA_int_bmux[13]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[13]), .XQ(XQA|partial_corrupt_A[13]), .Q(QA_int[13]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA14 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[13]), .D(DA_int_bmux[14]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[14]), .XQ(XQA|partial_corrupt_A[14]), .Q(QA_int[14]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA15 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[14]), .D(DA_int_bmux[15]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[15]), .XQ(XQA|partial_corrupt_A[15]), .Q(QA_int[15]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA16 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[15]), .D(DA_int_bmux[16]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[16]), .XQ(XQA|partial_corrupt_A[16]), .Q(QA_int[16]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA17 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[16]), .D(DA_int_bmux[17]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[17]), .XQ(XQA|partial_corrupt_A[17]), .Q(QA_int[17]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA18 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[17]), .D(DA_int_bmux[18]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[18]), .XQ(XQA|partial_corrupt_A[18]), .Q(QA_int[18]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA19 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[18]), .D(DA_int_bmux[19]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[19]), .XQ(XQA|partial_corrupt_A[19]), .Q(QA_int[19]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA20 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[19]), .D(DA_int_bmux[20]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[20]), .XQ(XQA|partial_corrupt_A[20]), .Q(QA_int[20]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA21 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[20]), .D(DA_int_bmux[21]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[21]), .XQ(XQA|partial_corrupt_A[21]), .Q(QA_int[21]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA22 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[21]), .D(DA_int_bmux[22]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[22]), .XQ(XQA|partial_corrupt_A[22]), .Q(QA_int[22]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA23 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[22]), .D(DA_int_bmux[23]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[23]), .XQ(XQA|partial_corrupt_A[23]), .Q(QA_int[23]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA24 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[23]), .D(DA_int_bmux[24]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[24]), .XQ(XQA|partial_corrupt_A[24]), .Q(QA_int[24]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA25 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[24]), .D(DA_int_bmux[25]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[25]), .XQ(XQA|partial_corrupt_A[25]), .Q(QA_int[25]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA26 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[25]), .D(DA_int_bmux[26]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[26]), .XQ(XQA|partial_corrupt_A[26]), .Q(QA_int[26]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA27 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[26]), .D(DA_int_bmux[27]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[27]), .XQ(XQA|partial_corrupt_A[27]), .Q(QA_int[27]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA28 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[27]), .D(DA_int_bmux[28]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[28]), .XQ(XQA|partial_corrupt_A[28]), .Q(QA_int[28]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA29 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[28]), .D(DA_int_bmux[29]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[29]), .XQ(XQA|partial_corrupt_A[29]), .Q(QA_int[29]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA30 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[29]), .D(DA_int_bmux[30]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[30]), .XQ(XQA|partial_corrupt_A[30]), .Q(QA_int[30]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA31 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[30]), .D(DA_int_bmux[31]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[31]), .XQ(XQA|partial_corrupt_A[31]), .Q(QA_int[31]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA32 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[33]), .D(DA_int_bmux[32]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[32]), .XQ(XQA|partial_corrupt_A[32]), .Q(QA_int[32]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA33 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[34]), .D(DA_int_bmux[33]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[33]), .XQ(XQA|partial_corrupt_A[33]), .Q(QA_int[33]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA34 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[35]), .D(DA_int_bmux[34]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[34]), .XQ(XQA|partial_corrupt_A[34]), .Q(QA_int[34]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA35 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[36]), .D(DA_int_bmux[35]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[35]), .XQ(XQA|partial_corrupt_A[35]), .Q(QA_int[35]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA36 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[37]), .D(DA_int_bmux[36]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[36]), .XQ(XQA|partial_corrupt_A[36]), .Q(QA_int[36]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA37 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[38]), .D(DA_int_bmux[37]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[37]), .XQ(XQA|partial_corrupt_A[37]), .Q(QA_int[37]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA38 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[39]), .D(DA_int_bmux[38]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[38]), .XQ(XQA|partial_corrupt_A[38]), .Q(QA_int[38]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA39 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[40]), .D(DA_int_bmux[39]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[39]), .XQ(XQA|partial_corrupt_A[39]), .Q(QA_int[39]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA40 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[41]), .D(DA_int_bmux[40]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[40]), .XQ(XQA|partial_corrupt_A[40]), .Q(QA_int[40]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA41 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[42]), .D(DA_int_bmux[41]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[41]), .XQ(XQA|partial_corrupt_A[41]), .Q(QA_int[41]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA42 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[43]), .D(DA_int_bmux[42]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[42]), .XQ(XQA|partial_corrupt_A[42]), .Q(QA_int[42]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA43 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[44]), .D(DA_int_bmux[43]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[43]), .XQ(XQA|partial_corrupt_A[43]), .Q(QA_int[43]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA44 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[45]), .D(DA_int_bmux[44]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[44]), .XQ(XQA|partial_corrupt_A[44]), .Q(QA_int[44]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA45 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[46]), .D(DA_int_bmux[45]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[45]), .XQ(XQA|partial_corrupt_A[45]), .Q(QA_int[45]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA46 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[47]), .D(DA_int_bmux[46]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[46]), .XQ(XQA|partial_corrupt_A[46]), .Q(QA_int[46]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA47 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[48]), .D(DA_int_bmux[47]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[47]), .XQ(XQA|partial_corrupt_A[47]), .Q(QA_int[47]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA48 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[49]), .D(DA_int_bmux[48]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[48]), .XQ(XQA|partial_corrupt_A[48]), .Q(QA_int[48]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA49 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[50]), .D(DA_int_bmux[49]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[49]), .XQ(XQA|partial_corrupt_A[49]), .Q(QA_int[49]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA50 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[51]), .D(DA_int_bmux[50]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[50]), .XQ(XQA|partial_corrupt_A[50]), .Q(QA_int[50]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA51 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[52]), .D(DA_int_bmux[51]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[51]), .XQ(XQA|partial_corrupt_A[51]), .Q(QA_int[51]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA52 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[53]), .D(DA_int_bmux[52]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[52]), .XQ(XQA|partial_corrupt_A[52]), .Q(QA_int[52]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA53 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[54]), .D(DA_int_bmux[53]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[53]), .XQ(XQA|partial_corrupt_A[53]), .Q(QA_int[53]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA54 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[55]), .D(DA_int_bmux[54]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[54]), .XQ(XQA|partial_corrupt_A[54]), .Q(QA_int[54]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA55 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[56]), .D(DA_int_bmux[55]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[55]), .XQ(XQA|partial_corrupt_A[55]), .Q(QA_int[55]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA56 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[57]), .D(DA_int_bmux[56]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[56]), .XQ(XQA|partial_corrupt_A[56]), .Q(QA_int[56]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA57 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[58]), .D(DA_int_bmux[57]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[57]), .XQ(XQA|partial_corrupt_A[57]), .Q(QA_int[57]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA58 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[59]), .D(DA_int_bmux[58]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[58]), .XQ(XQA|partial_corrupt_A[58]), .Q(QA_int[58]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA59 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[60]), .D(DA_int_bmux[59]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[59]), .XQ(XQA|partial_corrupt_A[59]), .Q(QA_int[59]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA60 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[61]), .D(DA_int_bmux[60]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[60]), .XQ(XQA|partial_corrupt_A[60]), .Q(QA_int[60]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA61 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[62]), .D(DA_int_bmux[61]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[61]), .XQ(XQA|partial_corrupt_A[61]), .Q(QA_int[61]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA62 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[63]), .D(DA_int_bmux[62]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[62]), .XQ(XQA|partial_corrupt_A[62]), .Q(QA_int[62]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQA63 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(SIA_int[1]), .D(DA_int_bmux[63]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[63]), .XQ(XQA|partial_corrupt_A[63]), .Q(QA_int[63]));
     ...
     if (GWENB_int !== 1)
      ...
      if( (isBitX(GWENB_int) && DFTRAMBYP_int!==1) || isBitX(DFTRAMBYP_int) ) begin
        ...
        DB_int = {64{1'bx}};
          ...
          writeEnable = ~ ( {64{GWENB_int}} | {WENB_int[63], WENB_int[62], WENB_int[61],
      ...
      if (GWENB_int !== 1'b1 || DFTRAMBYP_int === 1'b1 || DFTRAMBYP_int === 1'bx) begin
        row_mask =  ( {3'b000, writeEnable[63], 3'b000, writeEnable[62], 3'b000, writeEnable[61],
          ...
          3'b000, writeEnable[5], 3'b000, writeEnable[4], 3'b000, writeEnable[3], 3'b000, writeEnable[2],
          3'b000, writeEnable[1], 3'b000, writeEnable[0]} << mux_address);
        new_data =  ( {3'b000, DB_int[63], 3'b000, DB_int[62], 3'b000, DB_int[61],
      ...
      if (GWENB_int !== 1'b1 || DFTRAMBYP_int === 1'b1) begin
        ...
        mem_path_B = {shifted_readLatch1[63], shifted_readLatch1[62], shifted_readLatch1[61],
...
`endif
  ...
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB0 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(SIB_int[0]), .D(DB_int_bmux[0]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[0]), .XQ(XQB|partial_corrupt_A[0]), .Q(QB_int[0]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB1 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[0]), .D(DB_int_bmux[1]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[1]), .XQ(XQB|partial_corrupt_A[1]), .Q(QB_int[1]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB2 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[1]), .D(DB_int_bmux[2]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[2]), .XQ(XQB|partial_corrupt_A[2]), .Q(QB_int[2]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB3 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[2]), .D(DB_int_bmux[3]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[3]), .XQ(XQB|partial_corrupt_A[3]), .Q(QB_int[3]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB4 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[3]), .D(DB_int_bmux[4]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[4]), .XQ(XQB|partial_corrupt_A[4]), .Q(QB_int[4]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB5 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[4]), .D(DB_int_bmux[5]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[5]), .XQ(XQB|partial_corrupt_A[5]), .Q(QB_int[5]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB6 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[5]), .D(DB_int_bmux[6]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[6]), .XQ(XQB|partial_corrupt_A[6]), .Q(QB_int[6]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB7 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[6]), .D(DB_int_bmux[7]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[7]), .XQ(XQB|partial_corrupt_A[7]), .Q(QB_int[7]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB8 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[7]), .D(DB_int_bmux[8]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[8]), .XQ(XQB|partial_corrupt_A[8]), .Q(QB_int[8]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB9 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[8]), .D(DB_int_bmux[9]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[9]), .XQ(XQB|partial_corrupt_A[9]), .Q(QB_int[9]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB10 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[9]), .D(DB_int_bmux[10]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[10]), .XQ(XQB|partial_corrupt_A[10]), .Q(QB_int[10]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB11 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[10]), .D(DB_int_bmux[11]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[11]), .XQ(XQB|partial_corrupt_A[11]), .Q(QB_int[11]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB12 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[11]), .D(DB_int_bmux[12]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[12]), .XQ(XQB|partial_corrupt_A[12]), .Q(QB_int[12]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB13 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[12]), .D(DB_int_bmux[13]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[13]), .XQ(XQB|partial_corrupt_A[13]), .Q(QB_int[13]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB14 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[13]), .D(DB_int_bmux[14]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[14]), .XQ(XQB|partial_corrupt_A[14]), .Q(QB_int[14]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB15 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[14]), .D(DB_int_bmux[15]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[15]), .XQ(XQB|partial_corrupt_A[15]), .Q(QB_int[15]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB16 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[15]), .D(DB_int_bmux[16]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[16]), .XQ(XQB|partial_corrupt_A[16]), .Q(QB_int[16]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB17 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[16]), .D(DB_int_bmux[17]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[17]), .XQ(XQB|partial_corrupt_A[17]), .Q(QB_int[17]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB18 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[17]), .D(DB_int_bmux[18]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[18]), .XQ(XQB|partial_corrupt_A[18]), .Q(QB_int[18]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB19 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[18]), .D(DB_int_bmux[19]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[19]), .XQ(XQB|partial_corrupt_A[19]), .Q(QB_int[19]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB20 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[19]), .D(DB_int_bmux[20]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[20]), .XQ(XQB|partial_corrupt_A[20]), .Q(QB_int[20]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB21 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[20]), .D(DB_int_bmux[21]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[21]), .XQ(XQB|partial_corrupt_A[21]), .Q(QB_int[21]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB22 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[21]), .D(DB_int_bmux[22]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[22]), .XQ(XQB|partial_corrupt_A[22]), .Q(QB_int[22]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB23 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[22]), .D(DB_int_bmux[23]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[23]), .XQ(XQB|partial_corrupt_A[23]), .Q(QB_int[23]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB24 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[23]), .D(DB_int_bmux[24]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[24]), .XQ(XQB|partial_corrupt_A[24]), .Q(QB_int[24]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB25 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[24]), .D(DB_int_bmux[25]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[25]), .XQ(XQB|partial_corrupt_A[25]), .Q(QB_int[25]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB26 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[25]), .D(DB_int_bmux[26]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[26]), .XQ(XQB|partial_corrupt_A[26]), .Q(QB_int[26]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB27 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[26]), .D(DB_int_bmux[27]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[27]), .XQ(XQB|partial_corrupt_A[27]), .Q(QB_int[27]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB28 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[27]), .D(DB_int_bmux[28]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[28]), .XQ(XQB|partial_corrupt_A[28]), .Q(QB_int[28]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB29 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[28]), .D(DB_int_bmux[29]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[29]), .XQ(XQB|partial_corrupt_A[29]), .Q(QB_int[29]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB30 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[29]), .D(DB_int_bmux[30]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[30]), .XQ(XQB|partial_corrupt_A[30]), .Q(QB_int[30]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB31 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[30]), .D(DB_int_bmux[31]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[31]), .XQ(XQB|partial_corrupt_A[31]), .Q(QB_int[31]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB32 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[33]), .D(DB_int_bmux[32]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[32]), .XQ(XQB|partial_corrupt_A[32]), .Q(QB_int[32]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB33 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[34]), .D(DB_int_bmux[33]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[33]), .XQ(XQB|partial_corrupt_A[33]), .Q(QB_int[33]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB34 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[35]), .D(DB_int_bmux[34]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[34]), .XQ(XQB|partial_corrupt_A[34]), .Q(QB_int[34]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB35 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[36]), .D(DB_int_bmux[35]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[35]), .XQ(XQB|partial_corrupt_A[35]), .Q(QB_int[35]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB36 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[37]), .D(DB_int_bmux[36]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[36]), .XQ(XQB|partial_corrupt_A[36]), .Q(QB_int[36]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB37 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[38]), .D(DB_int_bmux[37]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[37]), .XQ(XQB|partial_corrupt_A[37]), .Q(QB_int[37]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB38 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[39]), .D(DB_int_bmux[38]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[38]), .XQ(XQB|partial_corrupt_A[38]), .Q(QB_int[38]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB39 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[40]), .D(DB_int_bmux[39]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[39]), .XQ(XQB|partial_corrupt_A[39]), .Q(QB_int[39]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB40 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[41]), .D(DB_int_bmux[40]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[40]), .XQ(XQB|partial_corrupt_A[40]), .Q(QB_int[40]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB41 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[42]), .D(DB_int_bmux[41]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[41]), .XQ(XQB|partial_corrupt_A[41]), .Q(QB_int[41]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB42 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[43]), .D(DB_int_bmux[42]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[42]), .XQ(XQB|partial_corrupt_A[42]), .Q(QB_int[42]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB43 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[44]), .D(DB_int_bmux[43]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[43]), .XQ(XQB|partial_corrupt_A[43]), .Q(QB_int[43]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB44 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[45]), .D(DB_int_bmux[44]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[44]), .XQ(XQB|partial_corrupt_A[44]), .Q(QB_int[44]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB45 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[46]), .D(DB_int_bmux[45]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[45]), .XQ(XQB|partial_corrupt_A[45]), .Q(QB_int[45]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB46 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[47]), .D(DB_int_bmux[46]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[46]), .XQ(XQB|partial_corrupt_A[46]), .Q(QB_int[46]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB47 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[48]), .D(DB_int_bmux[47]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[47]), .XQ(XQB|partial_corrupt_A[47]), .Q(QB_int[47]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB48 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[49]), .D(DB_int_bmux[48]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[48]), .XQ(XQB|partial_corrupt_A[48]), .Q(QB_int[48]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB49 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[50]), .D(DB_int_bmux[49]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[49]), .XQ(XQB|partial_corrupt_A[49]), .Q(QB_int[49]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB50 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[51]), .D(DB_int_bmux[50]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[50]), .XQ(XQB|partial_corrupt_A[50]), .Q(QB_int[50]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB51 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[52]), .D(DB_int_bmux[51]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[51]), .XQ(XQB|partial_corrupt_A[51]), .Q(QB_int[51]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB52 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[53]), .D(DB_int_bmux[52]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[52]), .XQ(XQB|partial_corrupt_A[52]), .Q(QB_int[52]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB53 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[54]), .D(DB_int_bmux[53]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[53]), .XQ(XQB|partial_corrupt_A[53]), .Q(QB_int[53]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB54 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[55]), .D(DB_int_bmux[54]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[54]), .XQ(XQB|partial_corrupt_A[54]), .Q(QB_int[54]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB55 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[56]), .D(DB_int_bmux[55]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[55]), .XQ(XQB|partial_corrupt_A[55]), .Q(QB_int[55]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB56 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[57]), .D(DB_int_bmux[56]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[56]), .XQ(XQB|partial_corrupt_A[56]), .Q(QB_int[56]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB57 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[58]), .D(DB_int_bmux[57]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[57]), .XQ(XQB|partial_corrupt_A[57]), .Q(QB_int[57]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB58 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[59]), .D(DB_int_bmux[58]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[58]), .XQ(XQB|partial_corrupt_A[58]), .Q(QB_int[58]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB59 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[60]), .D(DB_int_bmux[59]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[59]), .XQ(XQB|partial_corrupt_A[59]), .Q(QB_int[59]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB60 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[61]), .D(DB_int_bmux[60]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[60]), .XQ(XQB|partial_corrupt_A[60]), .Q(QB_int[60]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB61 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[62]), .D(DB_int_bmux[61]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[61]), .XQ(XQB|partial_corrupt_A[61]), .Q(QB_int[61]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB62 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[63]), .D(DB_int_bmux[62]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[62]), .XQ(XQB|partial_corrupt_A[62]), .Q(QB_int[62]));
  datapath_latch_arm28hkcpdpsram256x64m4 uDQB63 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(SIB_int[1]), .D(DB_int_bmux[63]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[63]), .XQ(XQB|partial_corrupt_A[63]), .Q(QB_int[63]));
...
`endif
  ...
  reg NOT_CENA, NOT_WENA63, NOT_WENA62, NOT_WENA61, NOT_WENA60, NOT_WENA59, NOT_WENA58;
  reg NOT_WENA57, NOT_WENA56, NOT_WENA55, NOT_WENA54, NOT_WENA53, NOT_WENA52, NOT_WENA51;
  reg NOT_WENA50, NOT_WENA49, NOT_WENA48, NOT_WENA47, NOT_WENA46, NOT_WENA45, NOT_WENA44;
  reg NOT_WENA43, NOT_WENA42, NOT_WENA41, NOT_WENA40, NOT_WENA39, NOT_WENA38, NOT_WENA37;
  reg NOT_WENA36, NOT_WENA35, NOT_WENA34, NOT_WENA33, NOT_WENA32, NOT_WENA31, NOT_WENA30;
  reg NOT_WENA29, NOT_WENA28, NOT_WENA27, NOT_WENA26, NOT_WENA25, NOT_WENA24, NOT_WENA23;
  reg NOT_WENA22, NOT_WENA21, NOT_WENA20, NOT_WENA19, NOT_WENA18, NOT_WENA17, NOT_WENA16;
  reg NOT_WENA15, NOT_WENA14, NOT_WENA13, NOT_WENA12, NOT_WENA11, NOT_WENA10, NOT_WENA9;
  reg NOT_WENA8, NOT_WENA7, NOT_WENA6, NOT_WENA5, NOT_WENA4, NOT_WENA3, NOT_WENA2;
  reg NOT_WENA1, NOT_WENA0, NOT_AA7, NOT_AA6, NOT_AA5, NOT_AA4, NOT_AA3, NOT_AA2, NOT_AA1;
  reg NOT_AA0, NOT_DA63, NOT_DA62, NOT_DA61, NOT_DA60, NOT_DA59, NOT_DA58, NOT_DA57;
  ...
  reg NOT_CENB, NOT_WENB63, NOT_WENB62, NOT_WENB61, NOT_WENB60, NOT_WENB59, NOT_WENB58;
  reg NOT_WENB57, NOT_WENB56, NOT_WENB55, NOT_WENB54, NOT_WENB53, NOT_WENB52, NOT_WENB51;
  reg NOT_WENB50, NOT_WENB49, NOT_WENB48, NOT_WENB47, NOT_WENB46, NOT_WENB45, NOT_WENB44;
  reg NOT_WENB43, NOT_WENB42, NOT_WENB41, NOT_WENB40, NOT_WENB39, NOT_WENB38, NOT_WENB37;
  reg NOT_WENB36, NOT_WENB35, NOT_WENB34, NOT_WENB33, NOT_WENB32, NOT_WENB31, NOT_WENB30;
  ...
  reg NOT_WENB1, NOT_WENB0, NOT_AB7, NOT_AB6, NOT_AB5, NOT_AB4, NOT_AB3, NOT_AB2, NOT_AB1;
  reg NOT_AB0, NOT_DB63, NOT_DB62, NOT_DB61, NOT_DB60, NOT_DB59, NOT_DB58, NOT_DB57;
  ...
  reg NOT_TWENA63, NOT_TWENA62, NOT_TWENA61, NOT_TWENA60, NOT_TWENA59, NOT_TWENA58;
  reg NOT_TWENA57, NOT_TWENA56, NOT_TWENA55, NOT_TWENA54, NOT_TWENA53, NOT_TWENA52;
  reg NOT_TWENA51, NOT_TWENA50, NOT_TWENA49, NOT_TWENA48, NOT_TWENA47, NOT_TWENA46;
  reg NOT_TWENA45, NOT_TWENA44, NOT_TWENA43, NOT_TWENA42, NOT_TWENA41, NOT_TWENA40;
  reg NOT_TWENA39, NOT_TWENA38, NOT_TWENA37, NOT_TWENA36, NOT_TWENA35, NOT_TWENA34;
  reg NOT_TWENA33, NOT_TWENA32, NOT_TWENA31, NOT_TWENA30, NOT_TWENA29, NOT_TWENA28;
  reg NOT_TWENA27, NOT_TWENA26, NOT_TWENA25, NOT_TWENA24, NOT_TWENA23, NOT_TWENA22;
  reg NOT_TWENA21, NOT_TWENA20, NOT_TWENA19, NOT_TWENA18, NOT_TWENA17, NOT_TWENA16;
  reg NOT_TWENA15, NOT_TWENA14, NOT_TWENA13, NOT_TWENA12, NOT_TWENA11, NOT_TWENA10;
  reg NOT_TWENA9, NOT_TWENA8, NOT_TWENA7, NOT_TWENA6, NOT_TWENA5, NOT_TWENA4, NOT_TWENA3;
  reg NOT_TWENA2, NOT_TWENA1, NOT_TWENA0, NOT_TAA7, NOT_TAA6, NOT_TAA5, NOT_TAA4, NOT_TAA3;
  ...
  reg NOT_TDA1, NOT_TDA0, NOT_TENB, NOT_TCENB, NOT_TWENB63, NOT_TWENB62, NOT_TWENB61;
  ...
  reg NOT_TWENB5, NOT_TWENB4, NOT_TWENB3, NOT_TWENB2, NOT_TWENB1, NOT_TWENB0, NOT_TAB7;
  reg NOT_TAB6, NOT_TAB5, NOT_TAB4, NOT_TAB3, NOT_TAB2, NOT_TAB1, NOT_TAB0, NOT_TDB63;
  reg NOT_TDB62, NOT_TDB61, NOT_TDB60, NOT_TDB59, NOT_TDB58, NOT_TDB57, NOT_TDB56;
 ...
 wire  CLKB_;
  ...
  buf B65(AYA[0], AYA_[0]);
  buf B66(AYA[1], AYA_[1]);
  buf B67(AYA[2], AYA_[2]);
  buf B68(AYA[3], AYA_[3]);
  buf B69(AYA[4], AYA_[4]);
  buf B70(AYA[5], AYA_[5]);
  buf B71(AYA[6], AYA_[6]);
  buf B72(AYA[7], AYA_[7]);
  buf B73(CENYB, CENYB_);
  buf B74(WENYB[0], WENYB_[0]);
  buf B75(WENYB[1], WENYB_[1]);
  buf B76(WENYB[2], WENYB_[2]);
  buf B77(WENYB[3], WENYB_[3]);
  buf B78(WENYB[4], WENYB_[4]);
  buf B79(WENYB[5], WENYB_[5]);
  buf B80(WENYB[6], WENYB_[6]);
  buf B81(WENYB[7], WENYB_[7]);
  buf B82(WENYB[8], WENYB_[8]);
  buf B83(WENYB[9], WENYB_[9]);
  buf B84(WENYB[10], WENYB_[10]);
  buf B85(WENYB[11], WENYB_[11]);
  buf B86(WENYB[12], WENYB_[12]);
  buf B87(WENYB[13], WENYB_[13]);
  buf B88(WENYB[14], WENYB_[14]);
  buf B89(WENYB[15], WENYB_[15]);
  buf B90(WENYB[16], WENYB_[16]);
  buf B91(WENYB[17], WENYB_[17]);
  buf B92(WENYB[18], WENYB_[18]);
  buf B93(WENYB[19], WENYB_[19]);
  buf B94(WENYB[20], WENYB_[20]);
  buf B95(WENYB[21], WENYB_[21]);
  buf B96(WENYB[22], WENYB_[22]);
  buf B97(WENYB[23], WENYB_[23]);
  buf B98(WENYB[24], WENYB_[24]);
  buf B99(WENYB[25], WENYB_[25]);
  buf B100(WENYB[26], WENYB_[26]);
  buf B101(WENYB[27], WENYB_[27]);
  buf B102(WENYB[28], WENYB_[28]);
  buf B103(WENYB[29], WENYB_[29]);
  buf B104(WENYB[30], WENYB_[30]);
  buf B105(WENYB[31], WENYB_[31]);
  buf B106(WENYB[32], WENYB_[32]);
  buf B107(WENYB[33], WENYB_[33]);
  buf B108(WENYB[34], WENYB_[34]);
  buf B109(WENYB[35], WENYB_[35]);
  buf B110(WENYB[36], WENYB_[36]);
  buf B111(WENYB[37], WENYB_[37]);
  buf B112(WENYB[38], WENYB_[38]);
  buf B113(WENYB[39], WENYB_[39]);
  buf B114(WENYB[40], WENYB_[40]);
  buf B115(WENYB[41], WENYB_[41]);
  buf B116(WENYB[42], WENYB_[42]);
  buf B117(WENYB[43], WENYB_[43]);
  buf B118(WENYB[44], WENYB_[44]);
  buf B119(WENYB[45], WENYB_[45]);
  buf B120(WENYB[46], WENYB_[46]);
  buf B121(WENYB[47], WENYB_[47]);
  buf B122(WENYB[48], WENYB_[48]);
  buf B123(WENYB[49], WENYB_[49]);
  buf B124(WENYB[50], WENYB_[50]);
  buf B125(WENYB[51], WENYB_[51]);
  buf B126(WENYB[52], WENYB_[52]);
  buf B127(WENYB[53], WENYB_[53]);
  buf B128(WENYB[54], WENYB_[54]);
  buf B129(WENYB[55], WENYB_[55]);
  buf B130(WENYB[56], WENYB_[56]);
  buf B131(WENYB[57], WENYB_[57]);
  buf B132(WENYB[58], WENYB_[58]);
  buf B133(WENYB[59], WENYB_[59]);
  buf B134(WENYB[60], WENYB_[60]);
  buf B135(WENYB[61], WENYB_[61]);
  buf B136(WENYB[62], WENYB_[62]);
  buf B137(WENYB[63], WENYB_[63]);
  ...
  buf B146(GWENYA, GWENYA_);
  buf B147(GWENYB, GWENYB_);
  buf B148(QA[0], QA_[0]);
  buf B149(QA[1], QA_[1]);
  buf B150(QA[2], QA_[2]);
  buf B151(QA[3], QA_[3]);
  ...
  buf B220(QB[8], QB_[8]);
  buf B221(QB[9], QB_[9]);
  buf B222(QB[10], QB_[10]);
  buf B223(QB[11], QB_[11]);
  buf B224(QB[12], QB_[12]);
  buf B225(QB[13], QB_[13]);
  buf B226(QB[14], QB_[14]);
  buf B227(QB[15], QB_[15]);
  buf B228(QB[16], QB_[16]);
  buf B229(QB[17], QB_[17]);
  buf B230(QB[18], QB_[18]);
  buf B231(QB[19], QB_[19]);
  buf B232(QB[20], QB_[20]);
  buf B233(QB[21], QB_[21]);
  buf B234(QB[22], QB_[22]);
  buf B235(QB[23], QB_[23]);
  buf B236(QB[24], QB_[24]);
  buf B237(QB[25], QB_[25]);
  buf B238(QB[26], QB_[26]);
  buf B239(QB[27], QB_[27]);
  buf B240(QB[28], QB_[28]);
  buf B241(QB[29], QB_[29]);
  buf B242(QB[30], QB_[30]);
  buf B243(QB[31], QB_[31]);
  buf B244(QB[32], QB_[32]);
  buf B245(QB[33], QB_[33]);
  buf B246(QB[34], QB_[34]);
  buf B247(QB[35], QB_[35]);
  buf B248(QB[36], QB_[36]);
  buf B249(QB[37], QB_[37]);
  buf B250(QB[38], QB_[38]);
  buf B251(QB[39], QB_[39]);
  buf B252(QB[40], QB_[40]);
  buf B253(QB[41], QB_[41]);
  buf B254(QB[42], QB_[42]);
  buf B255(QB[43], QB_[43]);
  buf B256(QB[44], QB_[44]);
  buf B257(QB[45], QB_[45]);
  buf B258(QB[46], QB_[46]);
  buf B259(QB[47], QB_[47]);
  buf B260(QB[48], QB_[48]);
  buf B261(QB[49], QB_[49]);
  buf B262(QB[50], QB_[50]);
  buf B263(QB[51], QB_[51]);
  buf B264(QB[52], QB_[52]);
  buf B265(QB[53], QB_[53]);
  buf B266(QB[54], QB_[54]);
  buf B267(QB[55], QB_[55]);
  buf B268(QB[56], QB_[56]);
  buf B269(QB[57], QB_[57]);
  buf B270(QB[58], QB_[58]);
  buf B271(QB[59], QB_[59]);
  buf B272(QB[60], QB_[60]);
  buf B273(QB[61], QB_[61]);
  buf B274(QB[62], QB_[62]);
  buf B275(QB[63], QB_[63]);
  buf B276(SOA[0], SOA_[0]);
  buf B277(SOA[1], SOA_[1]);
  buf B278(SOB[0], SOB_[0]);
  buf B279(SOB[1], SOB_[1]);
  buf B280(CLKA_, CLKA);
  buf B281(CENA_, CENA);
  buf B282(WENA_[0], WENA[0]);
  buf B283(WENA_[1], WENA[1]);
  buf B284(WENA_[2], WENA[2]);
  buf B285(WENA_[3], WENA[3]);
  buf B286(WENA_[4], WENA[4]);
  buf B287(WENA_[5], WENA[5]);
  buf B288(WENA_[6], WENA[6]);
  buf B289(WENA_[7], WENA[7]);
  buf B290(WENA_[8], WENA[8]);
  buf B291(WENA_[9], WENA[9]);
  buf B292(WENA_[10], WENA[10]);
  buf B293(WENA_[11], WENA[11]);
  buf B294(WENA_[12], WENA[12]);
  buf B295(WENA_[13], WENA[13]);
  buf B296(WENA_[14], WENA[14]);
  buf B297(WENA_[15], WENA[15]);
  buf B298(WENA_[16], WENA[16]);
  buf B299(WENA_[17], WENA[17]);
  buf B300(WENA_[18], WENA[18]);
  buf B301(WENA_[19], WENA[19]);
  buf B302(WENA_[20], WENA[20]);
  buf B303(WENA_[21], WENA[21]);
  buf B304(WENA_[22], WENA[22]);
  buf B305(WENA_[23], WENA[23]);
  buf B306(WENA_[24], WENA[24]);
  buf B307(WENA_[25], WENA[25]);
  buf B308(WENA_[26], WENA[26]);
  buf B309(WENA_[27], WENA[27]);
  buf B310(WENA_[28], WENA[28]);
  buf B311(WENA_[29], WENA[29]);
  buf B312(WENA_[30], WENA[30]);
  buf B313(WENA_[31], WENA[31]);
  buf B314(WENA_[32], WENA[32]);
  buf B315(WENA_[33], WENA[33]);
  buf B316(WENA_[34], WENA[34]);
  buf B317(WENA_[35], WENA[35]);
  buf B318(WENA_[36], WENA[36]);
  buf B319(WENA_[37], WENA[37]);
  buf B320(WENA_[38], WENA[38]);
  buf B321(WENA_[39], WENA[39]);
  buf B322(WENA_[40], WENA[40]);
  buf B323(WENA_[41], WENA[41]);
  buf B324(WENA_[42], WENA[42]);
  buf B325(WENA_[43], WENA[43]);
  buf B326(WENA_[44], WENA[44]);
  buf B327(WENA_[45], WENA[45]);
  buf B328(WENA_[46], WENA[46]);
  buf B329(WENA_[47], WENA[47]);
  buf B330(WENA_[48], WENA[48]);
  buf B331(WENA_[49], WENA[49]);
  buf B332(WENA_[50], WENA[50]);
  buf B333(WENA_[51], WENA[51]);
  buf B334(WENA_[52], WENA[52]);
  buf B335(WENA_[53], WENA[53]);
  buf B336(WENA_[54], WENA[54]);
  buf B337(WENA_[55], WENA[55]);
  buf B338(WENA_[56], WENA[56]);
  buf B339(WENA_[57], WENA[57]);
  buf B340(WENA_[58], WENA[58]);
  buf B341(WENA_[59], WENA[59]);
  buf B342(WENA_[60], WENA[60]);
  buf B343(WENA_[61], WENA[61]);
  buf B344(WENA_[62], WENA[62]);
  buf B345(WENA_[63], WENA[63]);
  buf B346(AA_[0], AA[0]);
  buf B347(AA_[1], AA[1]);
  buf B348(AA_[2], AA[2]);
  buf B349(AA_[3], AA[3]);
  ...
  buf B362(DA_[8], DA[8]);
  buf B363(DA_[9], DA[9]);
  buf B364(DA_[10], DA[10]);
  buf B365(DA_[11], DA[11]);
  buf B366(DA_[12], DA[12]);
  buf B367(DA_[13], DA[13]);
  buf B368(DA_[14], DA[14]);
  buf B369(DA_[15], DA[15]);
  buf B370(DA_[16], DA[16]);
  buf B371(DA_[17], DA[17]);
  buf B372(DA_[18], DA[18]);
  buf B373(DA_[19], DA[19]);
  buf B374(DA_[20], DA[20]);
  buf B375(DA_[21], DA[21]);
  buf B376(DA_[22], DA[22]);
  buf B377(DA_[23], DA[23]);
  buf B378(DA_[24], DA[24]);
  buf B379(DA_[25], DA[25]);
  buf B380(DA_[26], DA[26]);
  buf B381(DA_[27], DA[27]);
  buf B382(DA_[28], DA[28]);
  buf B383(DA_[29], DA[29]);
  buf B384(DA_[30], DA[30]);
  buf B385(DA_[31], DA[31]);
  buf B386(DA_[32], DA[32]);
  buf B387(DA_[33], DA[33]);
  buf B388(DA_[34], DA[34]);
  buf B389(DA_[35], DA[35]);
  buf B390(DA_[36], DA[36]);
  buf B391(DA_[37], DA[37]);
  buf B392(DA_[38], DA[38]);
  buf B393(DA_[39], DA[39]);
  buf B394(DA_[40], DA[40]);
  buf B395(DA_[41], DA[41]);
  buf B396(DA_[42], DA[42]);
  buf B397(DA_[43], DA[43]);
  buf B398(DA_[44], DA[44]);
  buf B399(DA_[45], DA[45]);
  buf B400(DA_[46], DA[46]);
  buf B401(DA_[47], DA[47]);
  buf B402(DA_[48], DA[48]);
  buf B403(DA_[49], DA[49]);
  buf B404(DA_[50], DA[50]);
  buf B405(DA_[51], DA[51]);
  buf B406(DA_[52], DA[52]);
  buf B407(DA_[53], DA[53]);
  buf B408(DA_[54], DA[54]);
  buf B409(DA_[55], DA[55]);
  buf B410(DA_[56], DA[56]);
  buf B411(DA_[57], DA[57]);
  buf B412(DA_[58], DA[58]);
  buf B413(DA_[59], DA[59]);
  buf B414(DA_[60], DA[60]);
  buf B415(DA_[61], DA[61]);
  buf B416(DA_[62], DA[62]);
  buf B417(DA_[63], DA[63]);
  buf B418(CLKB_, CLKB);
  buf B419(CENB_, CENB);
  buf B420(WENB_[0], WENB[0]);
  buf B421(WENB_[1], WENB[1]);
  buf B422(WENB_[2], WENB[2]);
  buf B423(WENB_[3], WENB[3]);
  buf B424(WENB_[4], WENB[4]);
  buf B425(WENB_[5], WENB[5]);
  buf B426(WENB_[6], WENB[6]);
  buf B427(WENB_[7], WENB[7]);
  buf B428(WENB_[8], WENB[8]);
  buf B429(WENB_[9], WENB[9]);
  buf B430(WENB_[10], WENB[10]);
  buf B431(WENB_[11], WENB[11]);
  buf B432(WENB_[12], WENB[12]);
  buf B433(WENB_[13], WENB[13]);
  buf B434(WENB_[14], WENB[14]);
  buf B435(WENB_[15], WENB[15]);
  buf B436(WENB_[16], WENB[16]);
  buf B437(WENB_[17], WENB[17]);
  buf B438(WENB_[18], WENB[18]);
  buf B439(WENB_[19], WENB[19]);
  buf B440(WENB_[20], WENB[20]);
  buf B441(WENB_[21], WENB[21]);
  buf B442(WENB_[22], WENB[22]);
  buf B443(WENB_[23], WENB[23]);
  buf B444(WENB_[24], WENB[24]);
  buf B445(WENB_[25], WENB[25]);
  buf B446(WENB_[26], WENB[26]);
  buf B447(WENB_[27], WENB[27]);
  buf B448(WENB_[28], WENB[28]);
  buf B449(WENB_[29], WENB[29]);
  buf B450(WENB_[30], WENB[30]);
  buf B451(WENB_[31], WENB[31]);
  buf B452(WENB_[32], WENB[32]);
  buf B453(WENB_[33], WENB[33]);
  buf B454(WENB_[34], WENB[34]);
  buf B455(WENB_[35], WENB[35]);
  buf B456(WENB_[36], WENB[36]);
  buf B457(WENB_[37], WENB[37]);
  buf B458(WENB_[38], WENB[38]);
  buf B459(WENB_[39], WENB[39]);
  buf B460(WENB_[40], WENB[40]);
  buf B461(WENB_[41], WENB[41]);
  buf B462(WENB_[42], WENB[42]);
  buf B463(WENB_[43], WENB[43]);
  buf B464(WENB_[44], WENB[44]);
  buf B465(WENB_[45], WENB[45]);
  buf B466(WENB_[46], WENB[46]);
  buf B467(WENB_[47], WENB[47]);
  buf B468(WENB_[48], WENB[48]);
  buf B469(WENB_[49], WENB[49]);
  buf B470(WENB_[50], WENB[50]);
  buf B471(WENB_[51], WENB[51]);
  buf B472(WENB_[52], WENB[52]);
  buf B473(WENB_[53], WENB[53]);
  buf B474(WENB_[54], WENB[54]);
  buf B475(WENB_[55], WENB[55]);
  buf B476(WENB_[56], WENB[56]);
  buf B477(WENB_[57], WENB[57]);
  buf B478(WENB_[58], WENB[58]);
  buf B479(WENB_[59], WENB[59]);
  buf B480(WENB_[60], WENB[60]);
  buf B481(WENB_[61], WENB[61]);
  buf B482(WENB_[62], WENB[62]);
  buf B483(WENB_[63], WENB[63]);
  buf B484(AB_[0], AB[0]);
  buf B485(AB_[1], AB[1]);
  buf B486(AB_[2], AB[2]);
  buf B487(AB_[3], AB[3]);
  ...
  buf B500(DB_[8], DB[8]);
  buf B501(DB_[9], DB[9]);
  buf B502(DB_[10], DB[10]);
  buf B503(DB_[11], DB[11]);
  buf B504(DB_[12], DB[12]);
  buf B505(DB_[13], DB[13]);
  buf B506(DB_[14], DB[14]);
  buf B507(DB_[15], DB[15]);
  buf B508(DB_[16], DB[16]);
  buf B509(DB_[17], DB[17]);
  buf B510(DB_[18], DB[18]);
  buf B511(DB_[19], DB[19]);
  buf B512(DB_[20], DB[20]);
  buf B513(DB_[21], DB[21]);
  buf B514(DB_[22], DB[22]);
  buf B515(DB_[23], DB[23]);
  buf B516(DB_[24], DB[24]);
  buf B517(DB_[25], DB[25]);
  buf B518(DB_[26], DB[26]);
  buf B519(DB_[27], DB[27]);
  buf B520(DB_[28], DB[28]);
  buf B521(DB_[29], DB[29]);
  buf B522(DB_[30], DB[30]);
  buf B523(DB_[31], DB[31]);
  buf B524(DB_[32], DB[32]);
  buf B525(DB_[33], DB[33]);
  buf B526(DB_[34], DB[34]);
  buf B527(DB_[35], DB[35]);
  buf B528(DB_[36], DB[36]);
  buf B529(DB_[37], DB[37]);
  buf B530(DB_[38], DB[38]);
  buf B531(DB_[39], DB[39]);
  buf B532(DB_[40], DB[40]);
  buf B533(DB_[41], DB[41]);
  buf B534(DB_[42], DB[42]);
  buf B535(DB_[43], DB[43]);
  buf B536(DB_[44], DB[44]);
  buf B537(DB_[45], DB[45]);
  buf B538(DB_[46], DB[46]);
  buf B539(DB_[47], DB[47]);
  buf B540(DB_[48], DB[48]);
  buf B541(DB_[49], DB[49]);
  buf B542(DB_[50], DB[50]);
  buf B543(DB_[51], DB[51]);
  buf B544(DB_[52], DB[52]);
  buf B545(DB_[53], DB[53]);
  buf B546(DB_[54], DB[54]);
  buf B547(DB_[55], DB[55]);
  buf B548(DB_[56], DB[56]);
  buf B549(DB_[57], DB[57]);
  buf B550(DB_[58], DB[58]);
  buf B551(DB_[59], DB[59]);
  buf B552(DB_[60], DB[60]);
  buf B553(DB_[61], DB[61]);
  buf B554(DB_[62], DB[62]);
  buf B555(DB_[63], DB[63]);
  buf B556(EMAA_[0], EMAA[0]);
  buf B557(EMAA_[1], EMAA[1]);
  buf B558(EMAA_[2], EMAA[2]);
  buf B559(EMAWA_[0], EMAWA[0]);
  buf B560(EMAWA_[1], EMAWA[1]);
  buf B561(EMASA_, EMASA);
  buf B562(EMAB_[0], EMAB[0]);
  buf B563(EMAB_[1], EMAB[1]);
  buf B564(EMAB_[2], EMAB[2]);
  buf B565(EMAWB_[0], EMAWB[0]);
  buf B566(EMAWB_[1], EMAWB[1]);
  buf B567(EMASB_, EMASB);
  buf B568(TENA_, TENA);
  buf B569(TCENA_, TCENA);
  buf B570(TWENA_[0], TWENA[0]);
  buf B571(TWENA_[1], TWENA[1]);
  buf B572(TWENA_[2], TWENA[2]);
  buf B573(TWENA_[3], TWENA[3]);
  buf B574(TWENA_[4], TWENA[4]);
  buf B575(TWENA_[5], TWENA[5]);
  buf B576(TWENA_[6], TWENA[6]);
  buf B577(TWENA_[7], TWENA[7]);
  buf B578(TWENA_[8], TWENA[8]);
  buf B579(TWENA_[9], TWENA[9]);
  buf B580(TWENA_[10], TWENA[10]);
  buf B581(TWENA_[11], TWENA[11]);
  buf B582(TWENA_[12], TWENA[12]);
  buf B583(TWENA_[13], TWENA[13]);
  buf B584(TWENA_[14], TWENA[14]);
  buf B585(TWENA_[15], TWENA[15]);
  buf B586(TWENA_[16], TWENA[16]);
  buf B587(TWENA_[17], TWENA[17]);
  buf B588(TWENA_[18], TWENA[18]);
  buf B589(TWENA_[19], TWENA[19]);
  buf B590(TWENA_[20], TWENA[20]);
  buf B591(TWENA_[21], TWENA[21]);
  buf B592(TWENA_[22], TWENA[22]);
  buf B593(TWENA_[23], TWENA[23]);
  buf B594(TWENA_[24], TWENA[24]);
  buf B595(TWENA_[25], TWENA[25]);
  buf B596(TWENA_[26], TWENA[26]);
  buf B597(TWENA_[27], TWENA[27]);
  buf B598(TWENA_[28], TWENA[28]);
  buf B599(TWENA_[29], TWENA[29]);
  buf B600(TWENA_[30], TWENA[30]);
  buf B601(TWENA_[31], TWENA[31]);
  buf B602(TWENA_[32], TWENA[32]);
  buf B603(TWENA_[33], TWENA[33]);
  buf B604(TWENA_[34], TWENA[34]);
  buf B605(TWENA_[35], TWENA[35]);
  buf B606(TWENA_[36], TWENA[36]);
  buf B607(TWENA_[37], TWENA[37]);
  buf B608(TWENA_[38], TWENA[38]);
  buf B609(TWENA_[39], TWENA[39]);
  buf B610(TWENA_[40], TWENA[40]);
  buf B611(TWENA_[41], TWENA[41]);
  buf B612(TWENA_[42], TWENA[42]);
  buf B613(TWENA_[43], TWENA[43]);
  buf B614(TWENA_[44], TWENA[44]);
  buf B615(TWENA_[45], TWENA[45]);
  buf B616(TWENA_[46], TWENA[46]);
  buf B617(TWENA_[47], TWENA[47]);
  buf B618(TWENA_[48], TWENA[48]);
  buf B619(TWENA_[49], TWENA[49]);
  buf B620(TWENA_[50], TWENA[50]);
  buf B621(TWENA_[51], TWENA[51]);
  buf B622(TWENA_[52], TWENA[52]);
  buf B623(TWENA_[53], TWENA[53]);
  buf B624(TWENA_[54], TWENA[54]);
  buf B625(TWENA_[55], TWENA[55]);
  buf B626(TWENA_[56], TWENA[56]);
  buf B627(TWENA_[57], TWENA[57]);
  buf B628(TWENA_[58], TWENA[58]);
  buf B629(TWENA_[59], TWENA[59]);
  buf B630(TWENA_[60], TWENA[60]);
  buf B631(TWENA_[61], TWENA[61]);
  buf B632(TWENA_[62], TWENA[62]);
  buf B633(TWENA_[63], TWENA[63]);
  buf B634(TAA_[0], TAA[0]);
  buf B635(TAA_[1], TAA[1]);
  buf B636(TAA_[2], TAA[2]);
  buf B637(TAA_[3], TAA[3]);
  buf B638(TAA_[4], TAA[4]);
  buf B639(TAA_[5], TAA[5]);
  buf B640(TAA_[6], TAA[6]);
  buf B641(TAA_[7], TAA[7]);
  buf B642(TDA_[0], TDA[0]);
  buf B643(TDA_[1], TDA[1]);
  buf B644(TDA_[2], TDA[2]);
  buf B645(TDA_[3], TDA[3]);
  buf B646(TDA_[4], TDA[4]);
  buf B647(TDA_[5], TDA[5]);
  buf B648(TDA_[6], TDA[6]);
  buf B649(TDA_[7], TDA[7]);
  buf B650(TDA_[8], TDA[8]);
  buf B651(TDA_[9], TDA[9]);
  buf B652(TDA_[10], TDA[10]);
  buf B653(TDA_[11], TDA[11]);
  buf B654(TDA_[12], TDA[12]);
  buf B655(TDA_[13], TDA[13]);
  buf B656(TDA_[14], TDA[14]);
  buf B657(TDA_[15], TDA[15]);
  buf B658(TDA_[16], TDA[16]);
  buf B659(TDA_[17], TDA[17]);
  buf B660(TDA_[18], TDA[18]);
  buf B661(TDA_[19], TDA[19]);
  buf B662(TDA_[20], TDA[20]);
  buf B663(TDA_[21], TDA[21]);
  buf B664(TDA_[22], TDA[22]);
  buf B665(TDA_[23], TDA[23]);
  buf B666(TDA_[24], TDA[24]);
  buf B667(TDA_[25], TDA[25]);
  buf B668(TDA_[26], TDA[26]);
  buf B669(TDA_[27], TDA[27]);
  buf B670(TDA_[28], TDA[28]);
  buf B671(TDA_[29], TDA[29]);
  buf B672(TDA_[30], TDA[30]);
  buf B673(TDA_[31], TDA[31]);
  buf B674(TDA_[32], TDA[32]);
  buf B675(TDA_[33], TDA[33]);
  buf B676(TDA_[34], TDA[34]);
  buf B677(TDA_[35], TDA[35]);
  buf B678(TDA_[36], TDA[36]);
  buf B679(TDA_[37], TDA[37]);
  buf B680(TDA_[38], TDA[38]);
  buf B681(TDA_[39], TDA[39]);
  buf B682(TDA_[40], TDA[40]);
  buf B683(TDA_[41], TDA[41]);
  buf B684(TDA_[42], TDA[42]);
  buf B685(TDA_[43], TDA[43]);
  buf B686(TDA_[44], TDA[44]);
  buf B687(TDA_[45], TDA[45]);
  buf B688(TDA_[46], TDA[46]);
  buf B689(TDA_[47], TDA[47]);
  buf B690(TDA_[48], TDA[48]);
  buf B691(TDA_[49], TDA[49]);
  buf B692(TDA_[50], TDA[50]);
  buf B693(TDA_[51], TDA[51]);
  buf B694(TDA_[52], TDA[52]);
  buf B695(TDA_[53], TDA[53]);
  buf B696(TDA_[54], TDA[54]);
  buf B697(TDA_[55], TDA[55]);
  buf B698(TDA_[56], TDA[56]);
  buf B699(TDA_[57], TDA[57]);
  buf B700(TDA_[58], TDA[58]);
  buf B701(TDA_[59], TDA[59]);
  buf B702(TDA_[60], TDA[60]);
  buf B703(TDA_[61], TDA[61]);
  buf B704(TDA_[62], TDA[62]);
  buf B705(TDA_[63], TDA[63]);
  buf B706(TENB_, TENB);
  buf B707(TCENB_, TCENB);
  buf B708(TWENB_[0], TWENB[0]);
  buf B709(TWENB_[1], TWENB[1]);
  buf B710(TWENB_[2], TWENB[2]);
  buf B711(TWENB_[3], TWENB[3]);
  buf B712(TWENB_[4], TWENB[4]);
  buf B713(TWENB_[5], TWENB[5]);
  buf B714(TWENB_[6], TWENB[6]);
  buf B715(TWENB_[7], TWENB[7]);
  buf B716(TWENB_[8], TWENB[8]);
  buf B717(TWENB_[9], TWENB[9]);
  buf B718(TWENB_[10], TWENB[10]);
  buf B719(TWENB_[11], TWENB[11]);
  buf B720(TWENB_[12], TWENB[12]);
  buf B721(TWENB_[13], TWENB[13]);
  buf B722(TWENB_[14], TWENB[14]);
  buf B723(TWENB_[15], TWENB[15]);
  buf B724(TWENB_[16], TWENB[16]);
  buf B725(TWENB_[17], TWENB[17]);
  buf B726(TWENB_[18], TWENB[18]);
  buf B727(TWENB_[19], TWENB[19]);
  buf B728(TWENB_[20], TWENB[20]);
  buf B729(TWENB_[21], TWENB[21]);
  buf B730(TWENB_[22], TWENB[22]);
  buf B731(TWENB_[23], TWENB[23]);
  buf B732(TWENB_[24], TWENB[24]);
  buf B733(TWENB_[25], TWENB[25]);
  buf B734(TWENB_[26], TWENB[26]);
  buf B735(TWENB_[27], TWENB[27]);
  buf B736(TWENB_[28], TWENB[28]);
  buf B737(TWENB_[29], TWENB[29]);
  buf B738(TWENB_[30], TWENB[30]);
  buf B739(TWENB_[31], TWENB[31]);
  buf B740(TWENB_[32], TWENB[32]);
  buf B741(TWENB_[33], TWENB[33]);
  buf B742(TWENB_[34], TWENB[34]);
  buf B743(TWENB_[35], TWENB[35]);
  buf B744(TWENB_[36], TWENB[36]);
  buf B745(TWENB_[37], TWENB[37]);
  buf B746(TWENB_[38], TWENB[38]);
  buf B747(TWENB_[39], TWENB[39]);
  buf B748(TWENB_[40], TWENB[40]);
  buf B749(TWENB_[41], TWENB[41]);
  buf B750(TWENB_[42], TWENB[42]);
  buf B751(TWENB_[43], TWENB[43]);
  buf B752(TWENB_[44], TWENB[44]);
  buf B753(TWENB_[45], TWENB[45]);
  buf B754(TWENB_[46], TWENB[46]);
  buf B755(TWENB_[47], TWENB[47]);
  buf B756(TWENB_[48], TWENB[48]);
  buf B757(TWENB_[49], TWENB[49]);
  buf B758(TWENB_[50], TWENB[50]);
  buf B759(TWENB_[51], TWENB[51]);
  buf B760(TWENB_[52], TWENB[52]);
  buf B761(TWENB_[53], TWENB[53]);
  buf B762(TWENB_[54], TWENB[54]);
  buf B763(TWENB_[55], TWENB[55]);
  buf B764(TWENB_[56], TWENB[56]);
  buf B765(TWENB_[57], TWENB[57]);
  buf B766(TWENB_[58], TWENB[58]);
  buf B767(TWENB_[59], TWENB[59]);
  buf B768(TWENB_[60], TWENB[60]);
  buf B769(TWENB_[61], TWENB[61]);
  buf B770(TWENB_[62], TWENB[62]);
  buf B771(TWENB_[63], TWENB[63]);
  buf B772(TAB_[0], TAB[0]);
  buf B773(TAB_[1], TAB[1]);
  buf B774(TAB_[2], TAB[2]);
  buf B775(TAB_[3], TAB[3]);
  buf B776(TAB_[4], TAB[4]);
  buf B777(TAB_[5], TAB[5]);
  buf B778(TAB_[6], TAB[6]);
  buf B779(TAB_[7], TAB[7]);
  buf B780(TDB_[0], TDB[0]);
  buf B781(TDB_[1], TDB[1]);
  buf B782(TDB_[2], TDB[2]);
  buf B783(TDB_[3], TDB[3]);
  buf B784(TDB_[4], TDB[4]);
  buf B785(TDB_[5], TDB[5]);
  buf B786(TDB_[6], TDB[6]);
  buf B787(TDB_[7], TDB[7]);
  buf B788(TDB_[8], TDB[8]);
  buf B789(TDB_[9], TDB[9]);
  buf B790(TDB_[10], TDB[10]);
  buf B791(TDB_[11], TDB[11]);
  buf B792(TDB_[12], TDB[12]);
  buf B793(TDB_[13], TDB[13]);
  buf B794(TDB_[14], TDB[14]);
  buf B795(TDB_[15], TDB[15]);
  buf B796(TDB_[16], TDB[16]);
  buf B797(TDB_[17], TDB[17]);
  buf B798(TDB_[18], TDB[18]);
  buf B799(TDB_[19], TDB[19]);
  buf B800(TDB_[20], TDB[20]);
  buf B801(TDB_[21], TDB[21]);
  buf B802(TDB_[22], TDB[22]);
  buf B803(TDB_[23], TDB[23]);
  buf B804(TDB_[24], TDB[24]);
  buf B805(TDB_[25], TDB[25]);
  buf B806(TDB_[26], TDB[26]);
  buf B807(TDB_[27], TDB[27]);
  buf B808(TDB_[28], TDB[28]);
  buf B809(TDB_[29], TDB[29]);
  buf B810(TDB_[30], TDB[30]);
  buf B811(TDB_[31], TDB[31]);
  buf B812(TDB_[32], TDB[32]);
  buf B813(TDB_[33], TDB[33]);
  buf B814(TDB_[34], TDB[34]);
  buf B815(TDB_[35], TDB[35]);
  buf B816(TDB_[36], TDB[36]);
  buf B817(TDB_[37], TDB[37]);
  buf B818(TDB_[38], TDB[38]);
  buf B819(TDB_[39], TDB[39]);
  buf B820(TDB_[40], TDB[40]);
  buf B821(TDB_[41], TDB[41]);
  buf B822(TDB_[42], TDB[42]);
  buf B823(TDB_[43], TDB[43]);
  buf B824(TDB_[44], TDB[44]);
  buf B825(TDB_[45], TDB[45]);
  buf B826(TDB_[46], TDB[46]);
  buf B827(TDB_[47], TDB[47]);
  buf B828(TDB_[48], TDB[48]);
  buf B829(TDB_[49], TDB[49]);
  buf B830(TDB_[50], TDB[50]);
  buf B831(TDB_[51], TDB[51]);
  buf B832(TDB_[52], TDB[52]);
  buf B833(TDB_[53], TDB[53]);
  buf B834(TDB_[54], TDB[54]);
  buf B835(TDB_[55], TDB[55]);
  buf B836(TDB_[56], TDB[56]);
  buf B837(TDB_[57], TDB[57]);
  buf B838(TDB_[58], TDB[58]);
  buf B839(TDB_[59], TDB[59]);
  buf B840(TDB_[60], TDB[60]);
  buf B841(TDB_[61], TDB[61]);
  buf B842(TDB_[62], TDB[62]);
  buf B843(TDB_[63], TDB[63]);
  buf B844(GWENA_, GWENA);
  buf B845(GWENB_, GWENB);
  buf B846(TGWENA_, TGWENA);
  buf B847(TGWENB_, TGWENB);
  buf B848(RET1N_, RET1N);
  buf B849(SIA_[0], SIA[0]);
  buf B850(SIA_[1], SIA[1]);
  buf B851(SEA_, SEA);
  buf B852(DFTRAMBYP_, DFTRAMBYP);
  buf B853(SIB_[0], SIB[0]);
  buf B854(SIB_[1], SIB[1]);
  buf B855(SEB_, SEB);
  buf B856(COLLDISN_, COLLDISN);

  assign CENYA_ = (RET1N_ | pre_charge_st) ? (DFTRAMBYP_ & (TENA_ ? CENA_ : TCENA_)) : 1'bx;
  assign WENYA_ = (RET1N_ | pre_charge_st) ? ({64{DFTRAMBYP_}} & (TENA_ ? WENA_ : TWENA_)) : {64{1'bx}};
  ...
  assign WENYB_ = (RET1N_ | pre_charge_st) ? ({64{DFTRAMBYP_}} & (TENB_ ? WENB_ : TWENB_)) : {64{1'bx}};
  ...
  assign GWENYB_ = (RET1N_ | pre_charge_st) ? (DFTRAMBYP_ & (TENB_ ? GWENB_ : TGWENB_)) : 1'bx;
   `ifdef ARM_FAULT_MODELING
     arm28hkcpdpsram256x64m4_error_injection u1(.CLK(CLKA_), .Q_out(QA_), .A(AA_int), .CEN(CENA_int), .DFTRAMBYP(DFTRAMBYP_int), .SE(SEA_int), .GWEN(GWENA_int), .WEN(WENA_int), .Q_in(QA_int));
  `else
  assign QA_ = (RET1N_ | pre_charge_st) ? ((QA_int)) : {64{1'bx}};
  `endif
  assign QB_ = (RET1N_ | pre_charge_st) ? ((QB_int)) : {64{1'bx}};
...
`endif
  ...
  always @ globalNotifier0 begin
    ...
    if (clk0_int === 1'bx || CENA_int === 1'bx) begin
     ...
     && row_contention(TENB_ ? AB_ : TAB_, AA_int, ({64{GWENA_int}}|WENA_int), TENB_ ? ({64{GWENB_}}|WENB_) : ({64{TGWENB_}}|TWENB_))) begin
      cont_flag0_int = 1'b0;
        if (col_contention(AA_int, AB_int)) begin
          COL_CC = 1;
        end
...
`endif
     ...
     || COLLDISN_int === 1'bx) && row_contention(TENB_ ? AB_ : TAB_, AA_int, ({64{GWENA_int}}|WENA_int), TENB_ ? ({64{GWENB_}}|WENB_) : ({64{TGWENB_}}|TWENB_))) 
     begin
      cont_flag0_int = 1'b0;
          ROW_CC = 1;
          READ_READ_1 = 0;
...
`endif
  ...
  always @ globalNotifier1 begin
    ...
    if (clk1_int === 1'bx || CENB_int === 1'bx) begin
     ...
     && row_contention(TENA_ ? AA_ : TAA_, AB_int, ({64{GWENB_int}}|WENB_int), TENA_ ? ({64{GWENA_}}|WENA_) : ({64{TGWENA_}}|TWENA_))) begin
      cont_flag1_int = 1'b0;
        if (col_contention(AA_int, AB_int)) begin
          COL_CC = 1;
        end
...
`endif
     ...
     || COLLDISN_int === 1'bx) && row_contention(TENA_ ? AA_ : TAA_, AB_int, ({64{GWENB_int}}|WENB_int), TENA_ ? ({64{GWENA_}}|WENA_) : ({64{TGWENA_}}|TWENA_))) 
     begin
      cont_flag1_int = 1'b0;
          ROW_CC = 1;
          READ_READ_1 = 0;
...
`endif
  ...
  begin
   ...
   wire contA_flag = (CENA_int !== 1'b1 && ((TENB_ ? CENB_ : TCENB_) !== 1'b1)) && ((COLLDISN_int === 1'b1 && is_contention(TENB_ ? AB_ : TAB_, AA_int, TENB_ ? ({64{GWENB_}}|WENB_) : ({64{TGWENB_}}|TWENB_), ({64{GWENA_int}}|WENA_int))) ||
              ((COLLDISN_int === 1'b0 || COLLDISN_int === 1'bx) && row_contention(TENB_ ? AB_ : TAB_, AA_int, TENB_ ? ({64{GWENB_}}|WENB_) : ({64{TGWENB_}}|TWENB_), ({64{GWENA_int}}|WENA_int))));
   wire contB_flag = (CENB_int !== 1'b1 && ((TENA_ ? CENA_ : TCENA_) !== 1'b1)) && ((COLLDISN_int === 1'b1 && is_contention(TENA_ ? AA_ : TAA_, AB_int, TENA_ ? ({64{GWENA_}}|WENA_) : ({64{TGWENA_}}|TWENA_), ({64{GWENB_int}}|WENB_int))) ||
              ((COLLDISN_int === 1'b0 || COLLDISN_int === 1'bx) && row_contention(TENA_ ? AA_ : TAA_, AB_int, TENA_ ? ({64{GWENA_}}|WENA_) : ({64{TGWENA_}}|TWENA_), ({64{GWENB_int}}|WENB_int))));
...
`endif
`timescale 1ns/1ps
module arm28hkcpdpsram256x64m4_error_injection (Q_out, Q_in, CLK, A, CEN, DFTRAMBYP, SE, WEN, GWEN);
   output [63:0] Q_out;
   input [63:0] Q_in;
   input CLK;
   input [7:0] A;
