...
//
//       Instance Name:              arm28hkcpdpsram2048x4m8
//       Words:                      2048
//       Bits:                       4
//       Mux:                        8
//       Drive:                      6
...
//       
//       Creation Date:  Tue Jul 15 14:03:33 2025
//       Version: 	r0p1
//
...
module datapath_latch_arm28hkcpdpsram2048x4m8 (CLK,Q_update,D_update,SE,SI,D,DFTRAMBYP,mem_path,XQ,Q);
...
endmodule // datapath_latch_arm28hkcpdpsram2048x4m8
...
`ifdef POWER_PINS
module arm28hkcpdpsram2048x4m8 (VDDCE, VDDPE, VSSE, CENYA, WENYA, AYA, CENYB, WENYB,
    ...
    SIB, SEB, COLLDISN);
`else
module arm28hkcpdpsram2048x4m8 (CENYA, WENYA, AYA, CENYB, WENYB, AYB, GWENYA, GWENYB,
...
`endif
 ...
 wire  CLKB_;
  ...
  assign `ARM_UD_DP AYA_ = (RET1N_ | pre_charge_st) ? ({11{DFTRAMBYP_}} & (TENA_ ? AA_ : TAA_)) : {11{1'bx}};
  ...
  assign `ARM_UD_DP AYB_ = (RET1N_ | pre_charge_st) ? ({11{DFTRAMBYP_}} & (TENB_ ? AB_ : TAB_)) : {11{1'bx}};
  ...
  assign `ARM_UD_DP GWENYB_ = (RET1N_ | pre_charge_st) ? (DFTRAMBYP_ & (TENB_ ? GWENB_ : TGWENB_)) : 1'bx;
   `ifdef ARM_FAULT_MODELING
     arm28hkcpdpsram2048x4m8_error_injection u1(.CLK(CLKA_), .Q_out(QA_), .A(AA_int), .CEN(CENA_int), .DFTRAMBYP(DFTRAMBYP_int), .SE(SEA_int), .GWEN(GWENA_int), .WEN(WENA_int), .Q_in(QA_int));
  `else
...
`endif
      ...
      row = mem[row_address];
        writeEnable = {4{1'b1}};
        row_mask =  ( {7'b0000000, writeEnable[3], 7'b0000000, writeEnable[2], 7'b0000000, writeEnable[1],
          7'b0000000, writeEnable[0]} << mux_address);
        new_data =  ( {7'b0000000, wordtemp[3], 7'b0000000, wordtemp[2], 7'b0000000, wordtemp[1],
...
`endif
      ...
      row = mem[row_address];
        ...
        mem_path_A = {shifted_readLatch0[6], shifted_readLatch0[4], shifted_readLatch0[2],
    ...
    if (GWENA_int !== 1'b1 && DFTRAMBYP_int=== 1'b0 && SEA_int === 1'bx) begin
     ...
     if (GWENA_int !== 1)
      ...
      if (GWENA_int !== 1'b1 || DFTRAMBYP_int === 1'b1 || DFTRAMBYP_int === 1'bx) begin
        row_mask =  ( {7'b0000000, writeEnable[3], 7'b0000000, writeEnable[2], 7'b0000000, writeEnable[1],
          7'b0000000, writeEnable[0]} << mux_address);
        new_data =  ( {7'b0000000, DA_int[3], 7'b0000000, DA_int[2], 7'b0000000, DA_int[1],
...
`endif
  ...
  datapath_latch_arm28hkcpdpsram2048x4m8 uDQA0 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(SIA_int[0]), .D(DA_int_bmux[0]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[0]), .XQ(XQA|partial_corrupt_A[0]), .Q(QA_int[0]));
  datapath_latch_arm28hkcpdpsram2048x4m8 uDQA1 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[0]), .D(DA_int_bmux[1]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[1]), .XQ(XQA|partial_corrupt_A[1]), .Q(QA_int[1]));
  datapath_latch_arm28hkcpdpsram2048x4m8 uDQA2 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[3]), .D(DA_int_bmux[2]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[2]), .XQ(XQA|partial_corrupt_A[2]), .Q(QA_int[2]));
  datapath_latch_arm28hkcpdpsram2048x4m8 uDQA3 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(SIA_int[1]), .D(DA_int_bmux[3]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[3]), .XQ(XQA|partial_corrupt_A[3]), .Q(QA_int[3]));
    ...
    if (GWENB_int !== 1'b1 && DFTRAMBYP_int=== 1'b0 && SEB_int === 1'bx) begin
     ...
     if (GWENB_int !== 1)
      ...
      if (GWENB_int !== 1'b1 || DFTRAMBYP_int === 1'b1 || DFTRAMBYP_int === 1'bx) begin
        row_mask =  ( {7'b0000000, writeEnable[3], 7'b0000000, writeEnable[2], 7'b0000000, writeEnable[1],
          7'b0000000, writeEnable[0]} << mux_address);
        new_data =  ( {7'b0000000, DB_int[3], 7'b0000000, DB_int[2], 7'b0000000, DB_int[1],
      ...
      if (GWENB_int !== 1'b1 || DFTRAMBYP_int === 1'b1) begin
        ...
        mem_path_B = {shifted_readLatch1[6], shifted_readLatch1[4], shifted_readLatch1[2],
...
`endif
  ...
  datapath_latch_arm28hkcpdpsram2048x4m8 uDQB0 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(SIB_int[0]), .D(DB_int_bmux[0]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[0]), .XQ(XQB|partial_corrupt_A[0]), .Q(QB_int[0]));
  datapath_latch_arm28hkcpdpsram2048x4m8 uDQB1 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[0]), .D(DB_int_bmux[1]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[1]), .XQ(XQB|partial_corrupt_A[1]), .Q(QB_int[1]));
  datapath_latch_arm28hkcpdpsram2048x4m8 uDQB2 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[3]), .D(DB_int_bmux[2]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[2]), .XQ(XQB|partial_corrupt_A[2]), .Q(QB_int[2]));
  datapath_latch_arm28hkcpdpsram2048x4m8 uDQB3 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(SIB_int[1]), .D(DB_int_bmux[3]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[3]), .XQ(XQB|partial_corrupt_A[3]), .Q(QB_int[3]));
...
`endif
  ...
  reg NOT_CENA, NOT_WENA3, NOT_WENA2, NOT_WENA1, NOT_WENA0, NOT_AA10, NOT_AA9, NOT_AA8;
  reg NOT_AA7, NOT_AA6, NOT_AA5, NOT_AA4, NOT_AA3, NOT_AA2, NOT_AA1, NOT_AA0, NOT_DA3;
  reg NOT_DA2, NOT_DA1, NOT_DA0, NOT_CENB, NOT_WENB3, NOT_WENB2, NOT_WENB1, NOT_WENB0;
  reg NOT_AB10, NOT_AB9, NOT_AB8, NOT_AB7, NOT_AB6, NOT_AB5, NOT_AB4, NOT_AB3, NOT_AB2;
  reg NOT_AB1, NOT_AB0, NOT_DB3, NOT_DB2, NOT_DB1, NOT_DB0, NOT_EMAA2, NOT_EMAA1, NOT_EMAA0;
  reg NOT_EMAWA1, NOT_EMAWA0, NOT_EMASA, NOT_EMAB2, NOT_EMAB1, NOT_EMAB0, NOT_EMAWB1;
  reg NOT_EMAWB0, NOT_EMASB, NOT_TENA, NOT_TCENA, NOT_TWENA3, NOT_TWENA2, NOT_TWENA1;
  reg NOT_TWENA0, NOT_TAA10, NOT_TAA9, NOT_TAA8, NOT_TAA7, NOT_TAA6, NOT_TAA5, NOT_TAA4;
  reg NOT_TAA3, NOT_TAA2, NOT_TAA1, NOT_TAA0, NOT_TDA3, NOT_TDA2, NOT_TDA1, NOT_TDA0;
  reg NOT_TENB, NOT_TCENB, NOT_TWENB3, NOT_TWENB2, NOT_TWENB1, NOT_TWENB0, NOT_TAB10;
  reg NOT_TAB9, NOT_TAB8, NOT_TAB7, NOT_TAB6, NOT_TAB5, NOT_TAB4, NOT_TAB3, NOT_TAB2;
  reg NOT_TAB1, NOT_TAB0, NOT_TDB3, NOT_TDB2, NOT_TDB1, NOT_TDB0, NOT_GWENA, NOT_GWENB;
 ...
 wire  CLKB_;
  ...
  buf B13(AYA[8], AYA_[8]);
  buf B14(AYA[9], AYA_[9]);
  buf B15(AYA[10], AYA_[10]);
  ...
  buf B30(AYB[9], AYB_[9]);
  buf B31(AYB[10], AYB_[10]);
  ...
  buf B40(QB[2], QB_[2]);
  buf B41(QB[3], QB_[3]);
  buf B42(SOA[0], SOA_[0]);
  buf B43(SOA[1], SOA_[1]);
  buf B44(SOB[0], SOB_[0]);
  buf B45(SOB[1], SOB_[1]);
  buf B46(CLKA_, CLKA);
  buf B47(CENA_, CENA);
  buf B48(WENA_[0], WENA[0]);
  buf B49(WENA_[1], WENA[1]);
  buf B50(WENA_[2], WENA[2]);
  buf B51(WENA_[3], WENA[3]);
  buf B52(AA_[0], AA[0]);
  buf B53(AA_[1], AA[1]);
  buf B54(AA_[2], AA[2]);
  buf B55(AA_[3], AA[3]);
  ...
  buf B67(CLKB_, CLKB);
  buf B68(CENB_, CENB);
  buf B69(WENB_[0], WENB[0]);
  buf B70(WENB_[1], WENB[1]);
  buf B71(WENB_[2], WENB[2]);
  buf B72(WENB_[3], WENB[3]);
  buf B73(AB_[0], AB[0]);
  buf B74(AB_[1], AB[1]);
  buf B75(AB_[2], AB[2]);
  buf B76(AB_[3], AB[3]);
  ...
  buf B81(AB_[8], AB[8]);
  buf B82(AB_[9], AB[9]);
  buf B83(AB_[10], AB[10]);
  buf B84(DB_[0], DB[0]);
  buf B85(DB_[1], DB[1]);
  buf B86(DB_[2], DB[2]);
  buf B87(DB_[3], DB[3]);
  buf B88(EMAA_[0], EMAA[0]);
  buf B89(EMAA_[1], EMAA[1]);
  buf B90(EMAA_[2], EMAA[2]);
  buf B91(EMAWA_[0], EMAWA[0]);
  buf B92(EMAWA_[1], EMAWA[1]);
  buf B93(EMASA_, EMASA);
  buf B94(EMAB_[0], EMAB[0]);
  buf B95(EMAB_[1], EMAB[1]);
  buf B96(EMAB_[2], EMAB[2]);
  buf B97(EMAWB_[0], EMAWB[0]);
  buf B98(EMAWB_[1], EMAWB[1]);
  buf B99(EMASB_, EMASB);
  buf B100(TENA_, TENA);
  buf B101(TCENA_, TCENA);
  buf B102(TWENA_[0], TWENA[0]);
  buf B103(TWENA_[1], TWENA[1]);
  buf B104(TWENA_[2], TWENA[2]);
  buf B105(TWENA_[3], TWENA[3]);
  buf B106(TAA_[0], TAA[0]);
  buf B107(TAA_[1], TAA[1]);
  buf B108(TAA_[2], TAA[2]);
  buf B109(TAA_[3], TAA[3]);
  buf B110(TAA_[4], TAA[4]);
  buf B111(TAA_[5], TAA[5]);
  buf B112(TAA_[6], TAA[6]);
  buf B113(TAA_[7], TAA[7]);
  buf B114(TAA_[8], TAA[8]);
  buf B115(TAA_[9], TAA[9]);
  buf B116(TAA_[10], TAA[10]);
  buf B117(TDA_[0], TDA[0]);
  buf B118(TDA_[1], TDA[1]);
  buf B119(TDA_[2], TDA[2]);
  buf B120(TDA_[3], TDA[3]);
  buf B121(TENB_, TENB);
  buf B122(TCENB_, TCENB);
  buf B123(TWENB_[0], TWENB[0]);
  buf B124(TWENB_[1], TWENB[1]);
  buf B125(TWENB_[2], TWENB[2]);
  buf B126(TWENB_[3], TWENB[3]);
  buf B127(TAB_[0], TAB[0]);
  buf B128(TAB_[1], TAB[1]);
  buf B129(TAB_[2], TAB[2]);
  buf B130(TAB_[3], TAB[3]);
  buf B131(TAB_[4], TAB[4]);
  buf B132(TAB_[5], TAB[5]);
  buf B133(TAB_[6], TAB[6]);
  buf B134(TAB_[7], TAB[7]);
  buf B135(TAB_[8], TAB[8]);
  buf B136(TAB_[9], TAB[9]);
  buf B137(TAB_[10], TAB[10]);
  buf B138(TDB_[0], TDB[0]);
  buf B139(TDB_[1], TDB[1]);
  buf B140(TDB_[2], TDB[2]);
  buf B141(TDB_[3], TDB[3]);
  buf B142(GWENA_, GWENA);
  buf B143(GWENB_, GWENB);
  buf B144(TGWENA_, TGWENA);
  buf B145(TGWENB_, TGWENB);
  buf B146(RET1N_, RET1N);
  buf B147(SIA_[0], SIA[0]);
  buf B148(SIA_[1], SIA[1]);
  buf B149(SEA_, SEA);
  buf B150(DFTRAMBYP_, DFTRAMBYP);
  buf B151(SIB_[0], SIB[0]);
  buf B152(SIB_[1], SIB[1]);
  buf B153(SEB_, SEB);
  buf B154(COLLDISN_, COLLDISN);
  ...
  assign AYA_ = (RET1N_ | pre_charge_st) ? ({11{DFTRAMBYP_}} & (TENA_ ? AA_ : TAA_)) : {11{1'bx}};
  ...
  assign AYB_ = (RET1N_ | pre_charge_st) ? ({11{DFTRAMBYP_}} & (TENB_ ? AB_ : TAB_)) : {11{1'bx}};
...
`endif
`timescale 1ns/1ps
module arm28hkcpdpsram2048x4m8_error_injection (Q_out, Q_in, CLK, A, CEN, DFTRAMBYP, SE, WEN, GWEN);
   output [3:0] Q_out;
   input [3:0] Q_in;
   input CLK;
   input [10:0] A;
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.SMARCHCHKBVCD_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(11'd126,2'd2,2'd1,2'd0);
   `endif
end
   task add_fault;
   ...
   //In order to inject fault in redundant column for Bit 2 to 3, column address
