...
/*
    ...
    Filename   : smic281prf128x4m4.v
    IP code    : S28HKCPHD1PRF
    Version    : V0P2A
    CreateDate : Jul 11, 2025 11:54:12 AM

    Verilog Model for High Speed One-Port SRAM
    SMIC 28 SFE Process

    Configuration: -instname smic281prf128x4m4 
                   -words 128 -bits 4 -mux 4 -bank 1
...
`endif
  ...
  begin
      mem_rd_rslt = {mem_latch[12], mem_latch[8], mem_latch[4], mem_latch[0]};      
  end    
  endfunction 
  ...
  begin
      ...
      data_new = {3'b000, data_real[3], 3'b000, data_real[2], 3'b000, data_real[1], 3'b000, data_real[0]} << col_address;
      pos = 16'b0001000100010001 << col_address;
...
`endif
  assign nf_inputs_z = (CLK_buf === 1'bz) || (CEN_buf === 1'bz) || (WEN_buf === 1'bz) || (A_buf[0] === 1'bz) || (A_buf[1] === 1'bz) || 
                ...
                (D_buf[0] === 1'bz) || (D_buf[1] === 1'bz) || (D_buf[2] === 1'bz) || (D_buf[3] === 1'bz) || (BWEN_buf[0] === 1'bz) || 
                (BWEN_buf[1] === 1'bz) || (BWEN_buf[2] === 1'bz) || (BWEN_buf[3] === 1'bz) || 1'b0;
...
`else
  ...
  always @(TM_noti) begin
    ...
    assign BWEN_tmp[3] = ^BWEN[3];`endif
endmodule
`endcelldefine
