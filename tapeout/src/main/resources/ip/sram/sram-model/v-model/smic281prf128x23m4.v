...
/*
    ...
    Filename   : smic281prf128x23m4.v
    IP code    : S28HKCPHD1PRF
    Version    : V0P2A
    CreateDate : Jul 19, 2025 4:36:48 PM

    Verilog Model for High Speed One-Port SRAM
    SMIC 28 SFE Process

    Configuration: -instname smic281prf128x23m4 
                   -words 128 -bits 23 -mux 4 -bank 1
...
`endif
  ...
  parameter D_WIDTH      = 23;  // data bits
  parameter MUX          = 4;  // mux
  parameter MEM_WIDTH    = 92; 
...
`endif
  ...
  begin
      mem_rd_rslt = {mem_latch[88], mem_latch[84], mem_latch[80], mem_latch[76], 
  ...
  begin
      ...
      data_new = {3'b000, data_real[22], 3'b000, data_real[21], 3'b000, data_real[20], 3'b000, data_real[19], 3'b000, data_real[18], 
      ...
      pos = 92'b00010001000100010001000100010001000100010001000100010001000100010001000100010001000100010001 << col_address;
...
`endif
  assign nf_inputs_z = (CLK_buf === 1'bz) || (CEN_buf === 1'bz) || (WEN_buf === 1'bz) || (A_buf[0] === 1'bz) || (A_buf[1] === 1'bz) || 
                ...
                (D_buf[20] === 1'bz) || (D_buf[21] === 1'bz) || (D_buf[22] === 1'bz) || (BWEN_buf[0] === 1'bz) || (BWEN_buf[1] === 1'bz) || 
...
`else
  ...
  always @(TM_noti) begin
    ...
    assign BWEN_tmp[22] = ^BWEN[22];`endif
endmodule
`endcelldefine
