...
//
//       Instance Name:              arm28hkcpdpsram128x13m4
//       Words:                      128
//       Bits:                       13
//       Mux:                        4
//       Drive:                      6
...
//       
//       Creation Date:  Tue Jul 15 13:37:41 2025
//       Version: 	r0p1
//
...
module datapath_latch_arm28hkcpdpsram128x13m4 (CLK,Q_update,D_update,SE,SI,D,DFTRAMBYP,mem_path,XQ,Q);
...
endmodule // datapath_latch_arm28hkcpdpsram128x13m4
...
`ifdef POWER_PINS
module arm28hkcpdpsram128x13m4 (VDDCE, VDDPE, VSSE, CENYA, WENYA, AYA, CENYB, WENYB,
    ...
    SIB, SEB, COLLDISN);
`else
module arm28hkcpdpsram128x13m4 (CENYA, WENYA, AYA, CENYB, WENYB, AYB, GWENYA, GWENYB,
...
`endif
  ...
  parameter WORDS = 128;
  parameter MUX = 4;
  parameter MEM_WIDTH = 52; // redun block size 4, 24 on left, 28 on right
...
`endif
  ...
  reg [51:0] data_out;
  reg [12:0] readLatch0;
  reg [12:0] shifted_readLatch0;
  reg  read_mux_sel0_p2;
  reg [12:0] readLatch1;
  reg [12:0] shifted_readLatch1;
  reg  read_mux_sel1_p2;
  reg LAST_CLKB;
  wire [12:0] QA_int;
  reg XQA, QA_update;
  reg XDA_sh, DA_sh_update;
  wire [12:0] DA_int_bmux;
  reg [12:0] mem_path_A;
  reg [12:0] partial_mask;
  reg [12:0] partial_mask_A;
  reg [12:0] partial_corrupt_A = 13'b0;
  wire [12:0] QB_int;
  reg XQB, QB_update;
  reg XDB_sh, DB_sh_update;
  wire [12:0] DB_int_bmux;
  reg [12:0] mem_path_B;
  reg [12:0] partial_mask_B;
  reg [12:0] partial_corrupt_B = 13'b0;
  reg [12:0] writeEnable;
  real previous_CLKA;
  real previous_CLKB;
 ...
 wire  CLKB_;
  ...
  assign `ARM_UD_DP WENYA_ = (RET1N_ | pre_charge_st) ? ({13{DFTRAMBYP_}} & (TENA_ ? WENA_ : TWENA_)) : {13{1'bx}};
  ...
  assign `ARM_UD_DP WENYB_ = (RET1N_ | pre_charge_st) ? ({13{DFTRAMBYP_}} & (TENB_ ? WENB_ : TWENB_)) : {13{1'bx}};
  ...
  assign `ARM_UD_DP GWENYB_ = (RET1N_ | pre_charge_st) ? (DFTRAMBYP_ & (TENB_ ? GWENB_ : TGWENB_)) : 1'bx;
   `ifdef ARM_FAULT_MODELING
     arm28hkcpdpsram128x13m4_error_injection u1(.CLK(CLKA_), .Q_out(QA_), .A(AA_int), .CEN(CENA_int), .DFTRAMBYP(DFTRAMBYP_int), .SE(SEA_int), .GWEN(GWENA_int), .WEN(WENA_int), .Q_in(QA_int));
  `else
  assign `ARM_UD_SEQ QA_ = (RET1N_ | pre_charge_st) ? ((QA_int)) : {13{1'bx}};
  `endif
  assign `ARM_UD_SEQ QB_ = (RET1N_ | pre_charge_st) ? ((QB_int)) : {13{1'bx}};
  assign `ARM_UD_DP SOA_ = (RET1N_ | pre_charge_st) ? ({QA_[6], QA_[5]}) : {2{1'bx}};
  assign `ARM_UD_DP SOB_ = (RET1N_ | pre_charge_st) ? ({QB_[6], QB_[5]}) : {2{1'bx}};
...
`endif
      ...
      row = mem[row_address];
        writeEnable = {13{1'b1}};
        row_mask =  ( {3'b000, writeEnable[12], 3'b000, writeEnable[11], 3'b000, writeEnable[10],
        ...
        new_data =  ( {3'b000, wordtemp[12], 3'b000, wordtemp[11], 3'b000, wordtemp[10],
...
`endif
      ...
      row = mem[row_address];
        ...
        readLatch0 = {data_out[48], data_out[44], data_out[40], data_out[36], data_out[32],
          data_out[28], data_out[24], data_out[20], data_out[16], data_out[12], data_out[8],
          data_out[4], data_out[0]};
        shifted_readLatch0 = readLatch0;
        mem_path_A = {shifted_readLatch0[12], shifted_readLatch0[11], shifted_readLatch0[10],
     ...
     if (GWENA_int !== 1)
      ...
      if( (isBitX(GWENA_int) && DFTRAMBYP_int!==1) || isBitX(DFTRAMBYP_int) ) begin
        ...
        DA_int = {13{1'bx}};
          ...
          writeEnable = ~ ( {13{GWENA_int}} | {WENA_int[12], WENA_int[11], WENA_int[10],
      ...
      if (GWENA_int !== 1'b1 || DFTRAMBYP_int === 1'b1 || DFTRAMBYP_int === 1'bx) begin
        row_mask =  ( {3'b000, writeEnable[12], 3'b000, writeEnable[11], 3'b000, writeEnable[10],
          ...
          3'b000, writeEnable[5], 3'b000, writeEnable[4], 3'b000, writeEnable[3], 3'b000, writeEnable[2],
          3'b000, writeEnable[1], 3'b000, writeEnable[0]} << mux_address);
        new_data =  ( {3'b000, DA_int[12], 3'b000, DA_int[11], 3'b000, DA_int[10],
...
`endif
    ...
    #0;
      if (((previous_CLKA == previous_CLKB)) && (CENA_int !== 1'b1 && CENB_int !== 1'b1 && DFTRAMBYP_ !== 1'b1) && COLLDISN_int === 1'b1 && row_contention(AA_int,
        AB_int, ({13{GWENA_int}}|WENA_int), ({13{GWENB_int}}|WENB_int))) begin
        ...
        if (GWENA_int !== 1'b1 && GWENB_int !== 1'b1) begin
	      if (is_contention(AA_int, AB_int, ({13{GWENA_int}}|WENA_int), ({13{GWENB_int}}|WENB_int))) begin
          ...
          DA_int = ({13{1'bx}} & partial_mask_A) | (~partial_mask_A & DA_int);
  	WENA_int =  (({13{1'bx}} & partial_mask_A) | WENA_int) ;
          readWriteA;
          DB_int = ({13{1'bx}} & partial_mask_A) | (~partial_mask_A & DB_int);
  	WENB_int =  (({13{1'bx}} & partial_mask_A) | WENB_int) ;
          readWriteB;
		...
		if (is_contention(AA_int, AB_int, ({13{GWENA_int}}|WENA_int), ({13{GWENB_int}}|WENB_int))) begin
         if((|WENA_int) == 1'b1) begin
          ...
          readWriteB;
          readWriteA;
          partial_mask = ~{WENA_int[12], WENA_int[11], WENA_int[10], WENA_int[9], WENA_int[8],
            WENA_int[7], WENA_int[6], WENA_int[5], WENA_int[4], WENA_int[3], WENA_int[2],
            WENA_int[1], WENA_int[0]};
        mem_path_B = (partial_mask & {13{1'bx}}) | (~partial_mask & mem_path_B);
        #0;
        QB_update = 1'b0;
        #0;
        QB_update = 1'b1;
		...
		if (is_contention(AA_int, AB_int, ({13{GWENA_int}}|WENA_int), ({13{GWENB_int}}|WENB_int))) begin
         if((|WENB_int) == 1'b1) begin
          ...
          readWriteB;
          readWriteA;
          partial_mask = ~{WENB_int[12], WENB_int[11], WENB_int[10], WENB_int[9], WENB_int[8],
            WENB_int[7], WENB_int[6], WENB_int[5], WENB_int[4], WENB_int[3], WENB_int[2],
            WENB_int[1], WENB_int[0]};
        mem_path_A = (partial_mask & {13{1'bx}}) | (~partial_mask & mem_path_A);
        #0;
        QA_update = 1'b0;
        #0;
        QA_update = 1'b1;
...
`endif
       ...
       === 1'bx)  && row_contention(AA_int, AB_int, ({13{GWENA_int}}|WENA_int), ({13{GWENB_int}}|WENB_int))) begin
          ROW_CC = 1;
          READ_READ_1 = 0;
          READ_WRITE_1 = 0;
          WRITE_WRITE_1 = 0;
        ...
        if (GWENB_int !== 1'b1 && (& WENB_int) !== 1'b1) begin
          ...
          readWriteB;
        XQB = 1'b1; QB_update = 1'b1;
        end else if (is_contention(AA_int, AB_int, ({13{GWENA_int}}|WENA_int), ({13{GWENB_int}}|WENB_int))) begin
...
`endif
  ...
  datapath_latch_arm28hkcpdpsram128x13m4 uDQA0 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(SIA_int[0]), .D(DA_int_bmux[0]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[0]), .XQ(XQA|partial_corrupt_A[0]), .Q(QA_int[0]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQA1 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[0]), .D(DA_int_bmux[1]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[1]), .XQ(XQA|partial_corrupt_A[1]), .Q(QA_int[1]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQA2 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[1]), .D(DA_int_bmux[2]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[2]), .XQ(XQA|partial_corrupt_A[2]), .Q(QA_int[2]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQA3 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[2]), .D(DA_int_bmux[3]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[3]), .XQ(XQA|partial_corrupt_A[3]), .Q(QA_int[3]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQA4 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[3]), .D(DA_int_bmux[4]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[4]), .XQ(XQA|partial_corrupt_A[4]), .Q(QA_int[4]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQA5 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[4]), .D(DA_int_bmux[5]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[5]), .XQ(XQA|partial_corrupt_A[5]), .Q(QA_int[5]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQA6 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[7]), .D(DA_int_bmux[6]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[6]), .XQ(XQA|partial_corrupt_A[6]), .Q(QA_int[6]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQA7 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[8]), .D(DA_int_bmux[7]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[7]), .XQ(XQA|partial_corrupt_A[7]), .Q(QA_int[7]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQA8 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[9]), .D(DA_int_bmux[8]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[8]), .XQ(XQA|partial_corrupt_A[8]), .Q(QA_int[8]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQA9 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[10]), .D(DA_int_bmux[9]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[9]), .XQ(XQA|partial_corrupt_A[9]), .Q(QA_int[9]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQA10 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[11]), .D(DA_int_bmux[10]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[10]), .XQ(XQA|partial_corrupt_A[10]), .Q(QA_int[10]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQA11 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[12]), .D(DA_int_bmux[11]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[11]), .XQ(XQA|partial_corrupt_A[11]), .Q(QA_int[11]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQA12 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(SIA_int[1]), .D(DA_int_bmux[12]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[12]), .XQ(XQA|partial_corrupt_A[12]), .Q(QA_int[12]));
     ...
     if (GWENB_int !== 1)
      ...
      if( (isBitX(GWENB_int) && DFTRAMBYP_int!==1) || isBitX(DFTRAMBYP_int) ) begin
        ...
        DB_int = {13{1'bx}};
          ...
          writeEnable = ~ ( {13{GWENB_int}} | {WENB_int[12], WENB_int[11], WENB_int[10],
      ...
      if (GWENB_int !== 1'b1 || DFTRAMBYP_int === 1'b1 || DFTRAMBYP_int === 1'bx) begin
        row_mask =  ( {3'b000, writeEnable[12], 3'b000, writeEnable[11], 3'b000, writeEnable[10],
          ...
          3'b000, writeEnable[5], 3'b000, writeEnable[4], 3'b000, writeEnable[3], 3'b000, writeEnable[2],
          3'b000, writeEnable[1], 3'b000, writeEnable[0]} << mux_address);
        new_data =  ( {3'b000, DB_int[12], 3'b000, DB_int[11], 3'b000, DB_int[10],
        ...
        data_out = (row >> (mux_address%4));
        readLatch1 = {data_out[48], data_out[44], data_out[40], data_out[36], data_out[32],
          data_out[28], data_out[24], data_out[20], data_out[16], data_out[12], data_out[8],
          data_out[4], data_out[0]};
      end
      if (GWENB_int !== 1'b1 || DFTRAMBYP_int === 1'b1) begin
        ...
        mem_path_B = {shifted_readLatch1[12], shifted_readLatch1[11], shifted_readLatch1[10],
...
`endif
  ...
  datapath_latch_arm28hkcpdpsram128x13m4 uDQB0 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(SIB_int[0]), .D(DB_int_bmux[0]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[0]), .XQ(XQB|partial_corrupt_A[0]), .Q(QB_int[0]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQB1 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[0]), .D(DB_int_bmux[1]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[1]), .XQ(XQB|partial_corrupt_A[1]), .Q(QB_int[1]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQB2 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[1]), .D(DB_int_bmux[2]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[2]), .XQ(XQB|partial_corrupt_A[2]), .Q(QB_int[2]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQB3 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[2]), .D(DB_int_bmux[3]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[3]), .XQ(XQB|partial_corrupt_A[3]), .Q(QB_int[3]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQB4 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[3]), .D(DB_int_bmux[4]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[4]), .XQ(XQB|partial_corrupt_A[4]), .Q(QB_int[4]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQB5 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[4]), .D(DB_int_bmux[5]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[5]), .XQ(XQB|partial_corrupt_A[5]), .Q(QB_int[5]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQB6 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[7]), .D(DB_int_bmux[6]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[6]), .XQ(XQB|partial_corrupt_A[6]), .Q(QB_int[6]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQB7 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[8]), .D(DB_int_bmux[7]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[7]), .XQ(XQB|partial_corrupt_A[7]), .Q(QB_int[7]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQB8 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[9]), .D(DB_int_bmux[8]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[8]), .XQ(XQB|partial_corrupt_A[8]), .Q(QB_int[8]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQB9 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[10]), .D(DB_int_bmux[9]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[9]), .XQ(XQB|partial_corrupt_A[9]), .Q(QB_int[9]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQB10 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[11]), .D(DB_int_bmux[10]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[10]), .XQ(XQB|partial_corrupt_A[10]), .Q(QB_int[10]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQB11 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[12]), .D(DB_int_bmux[11]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[11]), .XQ(XQB|partial_corrupt_A[11]), .Q(QB_int[11]));
  datapath_latch_arm28hkcpdpsram128x13m4 uDQB12 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(SIB_int[1]), .D(DB_int_bmux[12]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[12]), .XQ(XQB|partial_corrupt_A[12]), .Q(QB_int[12]));
...
`endif
  ...
  reg NOT_CENA, NOT_WENA12, NOT_WENA11, NOT_WENA10, NOT_WENA9, NOT_WENA8, NOT_WENA7;
  reg NOT_WENA6, NOT_WENA5, NOT_WENA4, NOT_WENA3, NOT_WENA2, NOT_WENA1, NOT_WENA0;
  reg NOT_AA6, NOT_AA5, NOT_AA4, NOT_AA3, NOT_AA2, NOT_AA1, NOT_AA0, NOT_DA12, NOT_DA11;
  reg NOT_DA10, NOT_DA9, NOT_DA8, NOT_DA7, NOT_DA6, NOT_DA5, NOT_DA4, NOT_DA3, NOT_DA2;
  reg NOT_DA1, NOT_DA0, NOT_CENB, NOT_WENB12, NOT_WENB11, NOT_WENB10, NOT_WENB9, NOT_WENB8;
  reg NOT_WENB7, NOT_WENB6, NOT_WENB5, NOT_WENB4, NOT_WENB3, NOT_WENB2, NOT_WENB1;
  reg NOT_WENB0, NOT_AB6, NOT_AB5, NOT_AB4, NOT_AB3, NOT_AB2, NOT_AB1, NOT_AB0, NOT_DB12;
  ...
  reg NOT_TENA, NOT_TCENA, NOT_TWENA12, NOT_TWENA11, NOT_TWENA10, NOT_TWENA9, NOT_TWENA8;
  reg NOT_TWENA7, NOT_TWENA6, NOT_TWENA5, NOT_TWENA4, NOT_TWENA3, NOT_TWENA2, NOT_TWENA1;
  reg NOT_TWENA0, NOT_TAA6, NOT_TAA5, NOT_TAA4, NOT_TAA3, NOT_TAA2, NOT_TAA1, NOT_TAA0;
  reg NOT_TDA12, NOT_TDA11, NOT_TDA10, NOT_TDA9, NOT_TDA8, NOT_TDA7, NOT_TDA6, NOT_TDA5;
  reg NOT_TDA4, NOT_TDA3, NOT_TDA2, NOT_TDA1, NOT_TDA0, NOT_TENB, NOT_TCENB, NOT_TWENB12;
  ...
  reg NOT_TAB4, NOT_TAB3, NOT_TAB2, NOT_TAB1, NOT_TAB0, NOT_TDB12, NOT_TDB11, NOT_TDB10;
 ...
 wire  CLKB_;
  ...
  buf B14(AYA[0], AYA_[0]);
  buf B15(AYA[1], AYA_[1]);
  buf B16(AYA[2], AYA_[2]);
  buf B17(AYA[3], AYA_[3]);
  buf B18(AYA[4], AYA_[4]);
  buf B19(AYA[5], AYA_[5]);
  buf B20(AYA[6], AYA_[6]);
  buf B21(CENYB, CENYB_);
  buf B22(WENYB[0], WENYB_[0]);
  buf B23(WENYB[1], WENYB_[1]);
  buf B24(WENYB[2], WENYB_[2]);
  buf B25(WENYB[3], WENYB_[3]);
  buf B26(WENYB[4], WENYB_[4]);
  buf B27(WENYB[5], WENYB_[5]);
  buf B28(WENYB[6], WENYB_[6]);
  buf B29(WENYB[7], WENYB_[7]);
  buf B30(WENYB[8], WENYB_[8]);
  buf B31(WENYB[9], WENYB_[9]);
  buf B32(WENYB[10], WENYB_[10]);
  buf B33(WENYB[11], WENYB_[11]);
  buf B34(WENYB[12], WENYB_[12]);
  buf B35(AYB[0], AYB_[0]);
  buf B36(AYB[1], AYB_[1]);
  buf B37(AYB[2], AYB_[2]);
  buf B38(AYB[3], AYB_[3]);
  buf B39(AYB[4], AYB_[4]);
  buf B40(AYB[5], AYB_[5]);
  buf B41(AYB[6], AYB_[6]);
  buf B42(GWENYA, GWENYA_);
  buf B43(GWENYB, GWENYB_);
  buf B44(QA[0], QA_[0]);
  buf B45(QA[1], QA_[1]);
  buf B46(QA[2], QA_[2]);
  buf B47(QA[3], QA_[3]);
  ...
  buf B52(QA[8], QA_[8]);
  buf B53(QA[9], QA_[9]);
  buf B54(QA[10], QA_[10]);
  buf B55(QA[11], QA_[11]);
  buf B56(QA[12], QA_[12]);
  buf B57(QB[0], QB_[0]);
  buf B58(QB[1], QB_[1]);
  buf B59(QB[2], QB_[2]);
  buf B60(QB[3], QB_[3]);
  ...
  buf B65(QB[8], QB_[8]);
  buf B66(QB[9], QB_[9]);
  buf B67(QB[10], QB_[10]);
  buf B68(QB[11], QB_[11]);
  buf B69(QB[12], QB_[12]);
  buf B70(SOA[0], SOA_[0]);
  buf B71(SOA[1], SOA_[1]);
  buf B72(SOB[0], SOB_[0]);
  buf B73(SOB[1], SOB_[1]);
  buf B74(CLKA_, CLKA);
  buf B75(CENA_, CENA);
  buf B76(WENA_[0], WENA[0]);
  buf B77(WENA_[1], WENA[1]);
  buf B78(WENA_[2], WENA[2]);
  buf B79(WENA_[3], WENA[3]);
  buf B80(WENA_[4], WENA[4]);
  buf B81(WENA_[5], WENA[5]);
  buf B82(WENA_[6], WENA[6]);
  buf B83(WENA_[7], WENA[7]);
  buf B84(WENA_[8], WENA[8]);
  buf B85(WENA_[9], WENA[9]);
  buf B86(WENA_[10], WENA[10]);
  buf B87(WENA_[11], WENA[11]);
  buf B88(WENA_[12], WENA[12]);
  buf B89(AA_[0], AA[0]);
  buf B90(AA_[1], AA[1]);
  buf B91(AA_[2], AA[2]);
  buf B92(AA_[3], AA[3]);
  ...
  buf B104(DA_[8], DA[8]);
  buf B105(DA_[9], DA[9]);
  buf B106(DA_[10], DA[10]);
  buf B107(DA_[11], DA[11]);
  buf B108(DA_[12], DA[12]);
  buf B109(CLKB_, CLKB);
  buf B110(CENB_, CENB);
  buf B111(WENB_[0], WENB[0]);
  buf B112(WENB_[1], WENB[1]);
  buf B113(WENB_[2], WENB[2]);
  buf B114(WENB_[3], WENB[3]);
  buf B115(WENB_[4], WENB[4]);
  buf B116(WENB_[5], WENB[5]);
  buf B117(WENB_[6], WENB[6]);
  buf B118(WENB_[7], WENB[7]);
  buf B119(WENB_[8], WENB[8]);
  buf B120(WENB_[9], WENB[9]);
  buf B121(WENB_[10], WENB[10]);
  buf B122(WENB_[11], WENB[11]);
  buf B123(WENB_[12], WENB[12]);
  buf B124(AB_[0], AB[0]);
  buf B125(AB_[1], AB[1]);
  buf B126(AB_[2], AB[2]);
  buf B127(AB_[3], AB[3]);
  ...
  buf B139(DB_[8], DB[8]);
  buf B140(DB_[9], DB[9]);
  buf B141(DB_[10], DB[10]);
  buf B142(DB_[11], DB[11]);
  buf B143(DB_[12], DB[12]);
  buf B144(EMAA_[0], EMAA[0]);
  buf B145(EMAA_[1], EMAA[1]);
  buf B146(EMAA_[2], EMAA[2]);
  buf B147(EMAWA_[0], EMAWA[0]);
  buf B148(EMAWA_[1], EMAWA[1]);
  buf B149(EMASA_, EMASA);
  buf B150(EMAB_[0], EMAB[0]);
  buf B151(EMAB_[1], EMAB[1]);
  buf B152(EMAB_[2], EMAB[2]);
  buf B153(EMAWB_[0], EMAWB[0]);
  buf B154(EMAWB_[1], EMAWB[1]);
  buf B155(EMASB_, EMASB);
  buf B156(TENA_, TENA);
  buf B157(TCENA_, TCENA);
  buf B158(TWENA_[0], TWENA[0]);
  buf B159(TWENA_[1], TWENA[1]);
  buf B160(TWENA_[2], TWENA[2]);
  buf B161(TWENA_[3], TWENA[3]);
  buf B162(TWENA_[4], TWENA[4]);
  buf B163(TWENA_[5], TWENA[5]);
  buf B164(TWENA_[6], TWENA[6]);
  buf B165(TWENA_[7], TWENA[7]);
  buf B166(TWENA_[8], TWENA[8]);
  buf B167(TWENA_[9], TWENA[9]);
  buf B168(TWENA_[10], TWENA[10]);
  buf B169(TWENA_[11], TWENA[11]);
  buf B170(TWENA_[12], TWENA[12]);
  buf B171(TAA_[0], TAA[0]);
  buf B172(TAA_[1], TAA[1]);
  buf B173(TAA_[2], TAA[2]);
  buf B174(TAA_[3], TAA[3]);
  buf B175(TAA_[4], TAA[4]);
  buf B176(TAA_[5], TAA[5]);
  buf B177(TAA_[6], TAA[6]);
  buf B178(TDA_[0], TDA[0]);
  buf B179(TDA_[1], TDA[1]);
  buf B180(TDA_[2], TDA[2]);
  buf B181(TDA_[3], TDA[3]);
  buf B182(TDA_[4], TDA[4]);
  buf B183(TDA_[5], TDA[5]);
  buf B184(TDA_[6], TDA[6]);
  buf B185(TDA_[7], TDA[7]);
  buf B186(TDA_[8], TDA[8]);
  buf B187(TDA_[9], TDA[9]);
  buf B188(TDA_[10], TDA[10]);
  buf B189(TDA_[11], TDA[11]);
  buf B190(TDA_[12], TDA[12]);
  buf B191(TENB_, TENB);
  buf B192(TCENB_, TCENB);
  buf B193(TWENB_[0], TWENB[0]);
  buf B194(TWENB_[1], TWENB[1]);
  buf B195(TWENB_[2], TWENB[2]);
  buf B196(TWENB_[3], TWENB[3]);
  buf B197(TWENB_[4], TWENB[4]);
  buf B198(TWENB_[5], TWENB[5]);
  buf B199(TWENB_[6], TWENB[6]);
  buf B200(TWENB_[7], TWENB[7]);
  buf B201(TWENB_[8], TWENB[8]);
  buf B202(TWENB_[9], TWENB[9]);
  buf B203(TWENB_[10], TWENB[10]);
  buf B204(TWENB_[11], TWENB[11]);
  buf B205(TWENB_[12], TWENB[12]);
  buf B206(TAB_[0], TAB[0]);
  buf B207(TAB_[1], TAB[1]);
  buf B208(TAB_[2], TAB[2]);
  buf B209(TAB_[3], TAB[3]);
  buf B210(TAB_[4], TAB[4]);
  buf B211(TAB_[5], TAB[5]);
  buf B212(TAB_[6], TAB[6]);
  buf B213(TDB_[0], TDB[0]);
  buf B214(TDB_[1], TDB[1]);
  buf B215(TDB_[2], TDB[2]);
  buf B216(TDB_[3], TDB[3]);
  buf B217(TDB_[4], TDB[4]);
  buf B218(TDB_[5], TDB[5]);
  buf B219(TDB_[6], TDB[6]);
  buf B220(TDB_[7], TDB[7]);
  buf B221(TDB_[8], TDB[8]);
  buf B222(TDB_[9], TDB[9]);
  buf B223(TDB_[10], TDB[10]);
  buf B224(TDB_[11], TDB[11]);
  buf B225(TDB_[12], TDB[12]);
  buf B226(GWENA_, GWENA);
  buf B227(GWENB_, GWENB);
  buf B228(TGWENA_, TGWENA);
  buf B229(TGWENB_, TGWENB);
  buf B230(RET1N_, RET1N);
  buf B231(SIA_[0], SIA[0]);
  buf B232(SIA_[1], SIA[1]);
  buf B233(SEA_, SEA);
  buf B234(DFTRAMBYP_, DFTRAMBYP);
  buf B235(SIB_[0], SIB[0]);
  buf B236(SIB_[1], SIB[1]);
  buf B237(SEB_, SEB);
  buf B238(COLLDISN_, COLLDISN);

  assign CENYA_ = (RET1N_ | pre_charge_st) ? (DFTRAMBYP_ & (TENA_ ? CENA_ : TCENA_)) : 1'bx;
  assign WENYA_ = (RET1N_ | pre_charge_st) ? ({13{DFTRAMBYP_}} & (TENA_ ? WENA_ : TWENA_)) : {13{1'bx}};
  ...
  assign WENYB_ = (RET1N_ | pre_charge_st) ? ({13{DFTRAMBYP_}} & (TENB_ ? WENB_ : TWENB_)) : {13{1'bx}};
  ...
  assign GWENYB_ = (RET1N_ | pre_charge_st) ? (DFTRAMBYP_ & (TENB_ ? GWENB_ : TGWENB_)) : 1'bx;
   `ifdef ARM_FAULT_MODELING
     arm28hkcpdpsram128x13m4_error_injection u1(.CLK(CLKA_), .Q_out(QA_), .A(AA_int), .CEN(CENA_int), .DFTRAMBYP(DFTRAMBYP_int), .SE(SEA_int), .GWEN(GWENA_int), .WEN(WENA_int), .Q_in(QA_int));
  `else
  assign QA_ = (RET1N_ | pre_charge_st) ? ((QA_int)) : {13{1'bx}};
  `endif
  assign QB_ = (RET1N_ | pre_charge_st) ? ((QB_int)) : {13{1'bx}};
  assign SOA_ = (RET1N_ | pre_charge_st) ? ({QA_[6], QA_[5]}) : {2{1'bx}};
  assign SOB_ = (RET1N_ | pre_charge_st) ? ({QB_[6], QB_[5]}) : {2{1'bx}};
...
`endif
  ...
  always @ globalNotifier0 begin
    ...
    if (clk0_int === 1'bx || CENA_int === 1'bx) begin
     ...
     && row_contention(TENB_ ? AB_ : TAB_, AA_int, ({13{GWENA_int}}|WENA_int), TENB_ ? ({13{GWENB_}}|WENB_) : ({13{TGWENB_}}|TWENB_))) begin
      cont_flag0_int = 1'b0;
        if (col_contention(AA_int, AB_int)) begin
          COL_CC = 1;
        end
...
`endif
     ...
     || COLLDISN_int === 1'bx) && row_contention(TENB_ ? AB_ : TAB_, AA_int, ({13{GWENA_int}}|WENA_int), TENB_ ? ({13{GWENB_}}|WENB_) : ({13{TGWENB_}}|TWENB_))) 
     begin
      cont_flag0_int = 1'b0;
          ROW_CC = 1;
          READ_READ_1 = 0;
...
`endif
  ...
  always @ globalNotifier1 begin
    ...
    if (clk1_int === 1'bx || CENB_int === 1'bx) begin
     ...
     && row_contention(TENA_ ? AA_ : TAA_, AB_int, ({13{GWENB_int}}|WENB_int), TENA_ ? ({13{GWENA_}}|WENA_) : ({13{TGWENA_}}|TWENA_))) begin
      cont_flag1_int = 1'b0;
        if (col_contention(AA_int, AB_int)) begin
          COL_CC = 1;
        end
...
`endif
     ...
     || COLLDISN_int === 1'bx) && row_contention(TENA_ ? AA_ : TAA_, AB_int, ({13{GWENB_int}}|WENB_int), TENA_ ? ({13{GWENA_}}|WENA_) : ({13{TGWENA_}}|TWENA_))) 
     begin
      cont_flag1_int = 1'b0;
          ROW_CC = 1;
          READ_READ_1 = 0;
...
`endif
  ...
  begin
   ...
   wire contA_flag = (CENA_int !== 1'b1 && ((TENB_ ? CENB_ : TCENB_) !== 1'b1)) && ((COLLDISN_int === 1'b1 && is_contention(TENB_ ? AB_ : TAB_, AA_int, TENB_ ? ({13{GWENB_}}|WENB_) : ({13{TGWENB_}}|TWENB_), ({13{GWENA_int}}|WENA_int))) ||
              ((COLLDISN_int === 1'b0 || COLLDISN_int === 1'bx) && row_contention(TENB_ ? AB_ : TAB_, AA_int, TENB_ ? ({13{GWENB_}}|WENB_) : ({13{TGWENB_}}|TWENB_), ({13{GWENA_int}}|WENA_int))));
   wire contB_flag = (CENB_int !== 1'b1 && ((TENA_ ? CENA_ : TCENA_) !== 1'b1)) && ((COLLDISN_int === 1'b1 && is_contention(TENA_ ? AA_ : TAA_, AB_int, TENA_ ? ({13{GWENA_}}|WENA_) : ({13{TGWENA_}}|TWENA_), ({13{GWENB_int}}|WENB_int))) ||
              ((COLLDISN_int === 1'b0 || COLLDISN_int === 1'bx) && row_contention(TENA_ ? AA_ : TAA_, AB_int, TENA_ ? ({13{GWENA_}}|WENA_) : ({13{TGWENA_}}|TWENA_), ({13{GWENB_int}}|WENB_int))));
...
`endif
`timescale 1ns/1ps
module arm28hkcpdpsram128x13m4_error_injection (Q_out, Q_in, CLK, A, CEN, DFTRAMBYP, SE, WEN, GWEN);
   output [12:0] Q_out;
   input [12:0] Q_in;
   input CLK;
   input [6:0] A;
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.SMARCHCHKBVCD_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(7'd2,4'd9,2'd1,2'd0);
   `endif
end
   task add_fault;
