...
//
//       Instance Name:              arm28hkcpdpsram64x32m4
//       Words:                      64
//       Bits:                       32
//       Mux:                        4
//       Drive:                      6
...
//       
//       Creation Date:  Tue Jul 15 11:39:07 2025
//       Version: 	r0p1
//
...
module datapath_latch_arm28hkcpdpsram64x32m4 (CLK,Q_update,D_update,SE,SI,D,DFTRAMBYP,mem_path,XQ,Q);
...
endmodule // datapath_latch_arm28hkcpdpsram64x32m4
...
`ifdef POWER_PINS
module arm28hkcpdpsram64x32m4 (VDDCE, VDDPE, VSSE, CENYA, WENYA, AYA, CENYB, WENYB,
    ...
    SIB, SEB, COLLDISN);
`else
module arm28hkcpdpsram64x32m4 (CENYA, WENYA, AYA, CENYB, WENYB, AYB, GWENYA, GWENYB,
...
`endif
  ...
  reg [31:0] mem_path_A;
  reg [31:0] partial_mask;
  reg [31:0] partial_mask_A;
  reg [31:0] partial_corrupt_A = 32'b0;
  wire [31:0] QB_int;
  reg XQB, QB_update;
  reg XDB_sh, DB_sh_update;
  wire [31:0] DB_int_bmux;
  reg [31:0] mem_path_B;
  reg [31:0] partial_mask_B;
  reg [31:0] partial_corrupt_B = 32'b0;
 ...
 wire  CLKB_;
  ...
  assign `ARM_UD_DP WENYA_ = (RET1N_ | pre_charge_st) ? ({32{DFTRAMBYP_}} & (TENA_ ? WENA_ : TWENA_)) : {32{1'bx}};
  ...
  assign `ARM_UD_DP WENYB_ = (RET1N_ | pre_charge_st) ? ({32{DFTRAMBYP_}} & (TENB_ ? WENB_ : TWENB_)) : {32{1'bx}};
  ...
  assign `ARM_UD_DP GWENYB_ = (RET1N_ | pre_charge_st) ? (DFTRAMBYP_ & (TENB_ ? GWENB_ : TGWENB_)) : 1'bx;
   `ifdef ARM_FAULT_MODELING
     arm28hkcpdpsram64x32m4_error_injection u1(.CLK(CLKA_), .Q_out(QA_), .A(AA_int), .CEN(CENA_int), .DFTRAMBYP(DFTRAMBYP_int), .SE(SEA_int), .GWEN(GWENA_int), .WEN(WENA_int), .Q_in(QA_int));
  `else
     ...
     if (GWENA_int !== 1)
      ...
      if( (isBitX(GWENA_int) && DFTRAMBYP_int!==1) || isBitX(DFTRAMBYP_int) ) begin
        ...
        DA_int = {32{1'bx}};
          ...
          writeEnable = ~ ( {32{GWENA_int}} | {WENA_int[31], WENA_int[30], WENA_int[29],
    ...
    #0;
      if (((previous_CLKA == previous_CLKB)) && (CENA_int !== 1'b1 && CENB_int !== 1'b1 && DFTRAMBYP_ !== 1'b1) && COLLDISN_int === 1'b1 && row_contention(AA_int,
        ...
        if (GWENA_int !== 1'b1 && GWENB_int !== 1'b1) begin
          ...
          DA_int = ({32{1'bx}} & partial_mask_A) | (~partial_mask_A & DA_int);
  	WENA_int =  (({32{1'bx}} & partial_mask_A) | WENA_int) ;
          readWriteA;
          DB_int = ({32{1'bx}} & partial_mask_A) | (~partial_mask_A & DB_int);
  	WENB_int =  (({32{1'bx}} & partial_mask_A) | WENB_int) ;
          readWriteB;
		...
		if (is_contention(AA_int, AB_int, ({32{GWENA_int}}|WENA_int), ({32{GWENB_int}}|WENB_int))) begin
         if((|WENA_int) == 1'b1) begin
          ...
          readWriteB;
          readWriteA;
          partial_mask = ~{WENA_int[31], WENA_int[30], WENA_int[29], WENA_int[28],
        ...
        mem_path_B = (partial_mask & {32{1'bx}}) | (~partial_mask & mem_path_B);
        #0;
        QB_update = 1'b0;
        #0;
        QB_update = 1'b1;
		...
		if (is_contention(AA_int, AB_int, ({32{GWENA_int}}|WENA_int), ({32{GWENB_int}}|WENB_int))) begin
         if((|WENB_int) == 1'b1) begin
          ...
          readWriteB;
          readWriteA;
          partial_mask = ~{WENB_int[31], WENB_int[30], WENB_int[29], WENB_int[28],
        ...
        mem_path_A = (partial_mask & {32{1'bx}}) | (~partial_mask & mem_path_A);
        #0;
        QA_update = 1'b0;
        #0;
        QA_update = 1'b1;
...
`endif
       ...
       === 1'bx)  && row_contention(AA_int, AB_int, ({32{GWENA_int}}|WENA_int), ({32{GWENB_int}}|WENB_int))) begin
          ROW_CC = 1;
          READ_READ_1 = 0;
          READ_WRITE_1 = 0;
          WRITE_WRITE_1 = 0;
...
`endif
  ...
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA0 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(SIA_int[0]), .D(DA_int_bmux[0]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[0]), .XQ(XQA|partial_corrupt_A[0]), .Q(QA_int[0]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA1 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[0]), .D(DA_int_bmux[1]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[1]), .XQ(XQA|partial_corrupt_A[1]), .Q(QA_int[1]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA2 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[1]), .D(DA_int_bmux[2]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[2]), .XQ(XQA|partial_corrupt_A[2]), .Q(QA_int[2]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA3 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[2]), .D(DA_int_bmux[3]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[3]), .XQ(XQA|partial_corrupt_A[3]), .Q(QA_int[3]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA4 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[3]), .D(DA_int_bmux[4]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[4]), .XQ(XQA|partial_corrupt_A[4]), .Q(QA_int[4]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA5 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[4]), .D(DA_int_bmux[5]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[5]), .XQ(XQA|partial_corrupt_A[5]), .Q(QA_int[5]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA6 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[5]), .D(DA_int_bmux[6]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[6]), .XQ(XQA|partial_corrupt_A[6]), .Q(QA_int[6]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA7 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[6]), .D(DA_int_bmux[7]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[7]), .XQ(XQA|partial_corrupt_A[7]), .Q(QA_int[7]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA8 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[7]), .D(DA_int_bmux[8]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[8]), .XQ(XQA|partial_corrupt_A[8]), .Q(QA_int[8]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA9 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[8]), .D(DA_int_bmux[9]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[9]), .XQ(XQA|partial_corrupt_A[9]), .Q(QA_int[9]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA10 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[9]), .D(DA_int_bmux[10]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[10]), .XQ(XQA|partial_corrupt_A[10]), .Q(QA_int[10]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA11 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[10]), .D(DA_int_bmux[11]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[11]), .XQ(XQA|partial_corrupt_A[11]), .Q(QA_int[11]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA12 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[11]), .D(DA_int_bmux[12]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[12]), .XQ(XQA|partial_corrupt_A[12]), .Q(QA_int[12]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA13 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[12]), .D(DA_int_bmux[13]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[13]), .XQ(XQA|partial_corrupt_A[13]), .Q(QA_int[13]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA14 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[13]), .D(DA_int_bmux[14]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[14]), .XQ(XQA|partial_corrupt_A[14]), .Q(QA_int[14]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA15 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[14]), .D(DA_int_bmux[15]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[15]), .XQ(XQA|partial_corrupt_A[15]), .Q(QA_int[15]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA16 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[17]), .D(DA_int_bmux[16]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[16]), .XQ(XQA|partial_corrupt_A[16]), .Q(QA_int[16]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA17 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[18]), .D(DA_int_bmux[17]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[17]), .XQ(XQA|partial_corrupt_A[17]), .Q(QA_int[17]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA18 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[19]), .D(DA_int_bmux[18]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[18]), .XQ(XQA|partial_corrupt_A[18]), .Q(QA_int[18]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA19 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[20]), .D(DA_int_bmux[19]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[19]), .XQ(XQA|partial_corrupt_A[19]), .Q(QA_int[19]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA20 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[21]), .D(DA_int_bmux[20]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[20]), .XQ(XQA|partial_corrupt_A[20]), .Q(QA_int[20]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA21 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[22]), .D(DA_int_bmux[21]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[21]), .XQ(XQA|partial_corrupt_A[21]), .Q(QA_int[21]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA22 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[23]), .D(DA_int_bmux[22]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[22]), .XQ(XQA|partial_corrupt_A[22]), .Q(QA_int[22]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA23 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[24]), .D(DA_int_bmux[23]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[23]), .XQ(XQA|partial_corrupt_A[23]), .Q(QA_int[23]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA24 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[25]), .D(DA_int_bmux[24]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[24]), .XQ(XQA|partial_corrupt_A[24]), .Q(QA_int[24]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA25 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[26]), .D(DA_int_bmux[25]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[25]), .XQ(XQA|partial_corrupt_A[25]), .Q(QA_int[25]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA26 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[27]), .D(DA_int_bmux[26]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[26]), .XQ(XQA|partial_corrupt_A[26]), .Q(QA_int[26]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA27 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[28]), .D(DA_int_bmux[27]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[27]), .XQ(XQA|partial_corrupt_A[27]), .Q(QA_int[27]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA28 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[29]), .D(DA_int_bmux[28]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[28]), .XQ(XQA|partial_corrupt_A[28]), .Q(QA_int[28]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA29 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[30]), .D(DA_int_bmux[29]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[29]), .XQ(XQA|partial_corrupt_A[29]), .Q(QA_int[29]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA30 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(QA_int[31]), .D(DA_int_bmux[30]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[30]), .XQ(XQA|partial_corrupt_A[30]), .Q(QA_int[30]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQA31 (.CLK(CLKA), .Q_update(QA_update), .D_update(DA_sh_update), .SE(SEA_), .SI(SIA_int[1]), .D(DA_int_bmux[31]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_A[31]), .XQ(XQA|partial_corrupt_A[31]), .Q(QA_int[31]));
...
`endif
  ...
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB0 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(SIB_int[0]), .D(DB_int_bmux[0]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[0]), .XQ(XQB|partial_corrupt_A[0]), .Q(QB_int[0]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB1 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[0]), .D(DB_int_bmux[1]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[1]), .XQ(XQB|partial_corrupt_A[1]), .Q(QB_int[1]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB2 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[1]), .D(DB_int_bmux[2]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[2]), .XQ(XQB|partial_corrupt_A[2]), .Q(QB_int[2]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB3 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[2]), .D(DB_int_bmux[3]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[3]), .XQ(XQB|partial_corrupt_A[3]), .Q(QB_int[3]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB4 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[3]), .D(DB_int_bmux[4]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[4]), .XQ(XQB|partial_corrupt_A[4]), .Q(QB_int[4]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB5 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[4]), .D(DB_int_bmux[5]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[5]), .XQ(XQB|partial_corrupt_A[5]), .Q(QB_int[5]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB6 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[5]), .D(DB_int_bmux[6]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[6]), .XQ(XQB|partial_corrupt_A[6]), .Q(QB_int[6]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB7 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[6]), .D(DB_int_bmux[7]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[7]), .XQ(XQB|partial_corrupt_A[7]), .Q(QB_int[7]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB8 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[7]), .D(DB_int_bmux[8]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[8]), .XQ(XQB|partial_corrupt_A[8]), .Q(QB_int[8]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB9 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[8]), .D(DB_int_bmux[9]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[9]), .XQ(XQB|partial_corrupt_A[9]), .Q(QB_int[9]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB10 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[9]), .D(DB_int_bmux[10]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[10]), .XQ(XQB|partial_corrupt_A[10]), .Q(QB_int[10]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB11 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[10]), .D(DB_int_bmux[11]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[11]), .XQ(XQB|partial_corrupt_A[11]), .Q(QB_int[11]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB12 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[11]), .D(DB_int_bmux[12]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[12]), .XQ(XQB|partial_corrupt_A[12]), .Q(QB_int[12]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB13 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[12]), .D(DB_int_bmux[13]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[13]), .XQ(XQB|partial_corrupt_A[13]), .Q(QB_int[13]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB14 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[13]), .D(DB_int_bmux[14]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[14]), .XQ(XQB|partial_corrupt_A[14]), .Q(QB_int[14]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB15 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[14]), .D(DB_int_bmux[15]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[15]), .XQ(XQB|partial_corrupt_A[15]), .Q(QB_int[15]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB16 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[17]), .D(DB_int_bmux[16]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[16]), .XQ(XQB|partial_corrupt_A[16]), .Q(QB_int[16]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB17 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[18]), .D(DB_int_bmux[17]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[17]), .XQ(XQB|partial_corrupt_A[17]), .Q(QB_int[17]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB18 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[19]), .D(DB_int_bmux[18]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[18]), .XQ(XQB|partial_corrupt_A[18]), .Q(QB_int[18]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB19 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[20]), .D(DB_int_bmux[19]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[19]), .XQ(XQB|partial_corrupt_A[19]), .Q(QB_int[19]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB20 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[21]), .D(DB_int_bmux[20]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[20]), .XQ(XQB|partial_corrupt_A[20]), .Q(QB_int[20]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB21 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[22]), .D(DB_int_bmux[21]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[21]), .XQ(XQB|partial_corrupt_A[21]), .Q(QB_int[21]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB22 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[23]), .D(DB_int_bmux[22]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[22]), .XQ(XQB|partial_corrupt_A[22]), .Q(QB_int[22]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB23 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[24]), .D(DB_int_bmux[23]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[23]), .XQ(XQB|partial_corrupt_A[23]), .Q(QB_int[23]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB24 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[25]), .D(DB_int_bmux[24]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[24]), .XQ(XQB|partial_corrupt_A[24]), .Q(QB_int[24]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB25 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[26]), .D(DB_int_bmux[25]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[25]), .XQ(XQB|partial_corrupt_A[25]), .Q(QB_int[25]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB26 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[27]), .D(DB_int_bmux[26]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[26]), .XQ(XQB|partial_corrupt_A[26]), .Q(QB_int[26]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB27 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[28]), .D(DB_int_bmux[27]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[27]), .XQ(XQB|partial_corrupt_A[27]), .Q(QB_int[27]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB28 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[29]), .D(DB_int_bmux[28]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[28]), .XQ(XQB|partial_corrupt_A[28]), .Q(QB_int[28]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB29 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[30]), .D(DB_int_bmux[29]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[29]), .XQ(XQB|partial_corrupt_A[29]), .Q(QB_int[29]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB30 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(QB_int[31]), .D(DB_int_bmux[30]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[30]), .XQ(XQB|partial_corrupt_A[30]), .Q(QB_int[30]));
  datapath_latch_arm28hkcpdpsram64x32m4 uDQB31 (.CLK(CLKB), .Q_update(QB_update), .D_update(DB_sh_update), .SE(SEB_), .SI(SIB_int[1]), .D(DB_int_bmux[31]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path_B[31]), .XQ(XQB|partial_corrupt_A[31]), .Q(QB_int[31]));
...
`endif
  ...
  reg NOT_CENA, NOT_WENA31, NOT_WENA30, NOT_WENA29, NOT_WENA28, NOT_WENA27, NOT_WENA26;
  reg NOT_WENA25, NOT_WENA24, NOT_WENA23, NOT_WENA22, NOT_WENA21, NOT_WENA20, NOT_WENA19;
  reg NOT_WENA18, NOT_WENA17, NOT_WENA16, NOT_WENA15, NOT_WENA14, NOT_WENA13, NOT_WENA12;
  reg NOT_WENA11, NOT_WENA10, NOT_WENA9, NOT_WENA8, NOT_WENA7, NOT_WENA6, NOT_WENA5;
  reg NOT_WENA4, NOT_WENA3, NOT_WENA2, NOT_WENA1, NOT_WENA0, NOT_AA5, NOT_AA4, NOT_AA3;
  reg NOT_AA2, NOT_AA1, NOT_AA0, NOT_DA31, NOT_DA30, NOT_DA29, NOT_DA28, NOT_DA27;
  ...
  reg NOT_DA1, NOT_DA0, NOT_CENB, NOT_WENB31, NOT_WENB30, NOT_WENB29, NOT_WENB28, NOT_WENB27;
  ...
  reg NOT_AB3, NOT_AB2, NOT_AB1, NOT_AB0, NOT_DB31, NOT_DB30, NOT_DB29, NOT_DB28, NOT_DB27;
  ...
  reg NOT_TCENA, NOT_TWENA31, NOT_TWENA30, NOT_TWENA29, NOT_TWENA28, NOT_TWENA27, NOT_TWENA26;
  ...
  reg NOT_TAA5, NOT_TAA4, NOT_TAA3, NOT_TAA2, NOT_TAA1, NOT_TAA0, NOT_TDA31, NOT_TDA30;
  ...
  reg NOT_TENB, NOT_TCENB, NOT_TWENB31, NOT_TWENB30, NOT_TWENB29, NOT_TWENB28, NOT_TWENB27;
  reg NOT_TWENB26, NOT_TWENB25, NOT_TWENB24, NOT_TWENB23, NOT_TWENB22, NOT_TWENB21;
  reg NOT_TWENB20, NOT_TWENB19, NOT_TWENB18, NOT_TWENB17, NOT_TWENB16, NOT_TWENB15;
  reg NOT_TWENB14, NOT_TWENB13, NOT_TWENB12, NOT_TWENB11, NOT_TWENB10, NOT_TWENB9;
  reg NOT_TWENB8, NOT_TWENB7, NOT_TWENB6, NOT_TWENB5, NOT_TWENB4, NOT_TWENB3, NOT_TWENB2;
  reg NOT_TWENB1, NOT_TWENB0, NOT_TAB5, NOT_TAB4, NOT_TAB3, NOT_TAB2, NOT_TAB1, NOT_TAB0;
 ...
 wire  CLKB_;
  ...
  buf B33(AYA[0], AYA_[0]);
  buf B34(AYA[1], AYA_[1]);
  buf B35(AYA[2], AYA_[2]);
  buf B36(AYA[3], AYA_[3]);
  buf B37(AYA[4], AYA_[4]);
  buf B38(AYA[5], AYA_[5]);
  ...
  buf B71(WENYB[31], WENYB_[31]);
  buf B72(AYB[0], AYB_[0]);
  buf B73(AYB[1], AYB_[1]);
  buf B74(AYB[2], AYB_[2]);
  buf B75(AYB[3], AYB_[3]);
  buf B76(AYB[4], AYB_[4]);
  buf B77(AYB[5], AYB_[5]);
  ...
  buf B120(QB[8], QB_[8]);
  buf B121(QB[9], QB_[9]);
  buf B122(QB[10], QB_[10]);
  buf B123(QB[11], QB_[11]);
  buf B124(QB[12], QB_[12]);
  buf B125(QB[13], QB_[13]);
  buf B126(QB[14], QB_[14]);
  buf B127(QB[15], QB_[15]);
  buf B128(QB[16], QB_[16]);
  buf B129(QB[17], QB_[17]);
  buf B130(QB[18], QB_[18]);
  buf B131(QB[19], QB_[19]);
  buf B132(QB[20], QB_[20]);
  buf B133(QB[21], QB_[21]);
  buf B134(QB[22], QB_[22]);
  buf B135(QB[23], QB_[23]);
  buf B136(QB[24], QB_[24]);
  buf B137(QB[25], QB_[25]);
  buf B138(QB[26], QB_[26]);
  buf B139(QB[27], QB_[27]);
  buf B140(QB[28], QB_[28]);
  buf B141(QB[29], QB_[29]);
  buf B142(QB[30], QB_[30]);
  buf B143(QB[31], QB_[31]);
  buf B144(SOA[0], SOA_[0]);
  buf B145(SOA[1], SOA_[1]);
  buf B146(SOB[0], SOB_[0]);
  buf B147(SOB[1], SOB_[1]);
  buf B148(CLKA_, CLKA);
  buf B149(CENA_, CENA);
  buf B150(WENA_[0], WENA[0]);
  buf B151(WENA_[1], WENA[1]);
  buf B152(WENA_[2], WENA[2]);
  buf B153(WENA_[3], WENA[3]);
  buf B154(WENA_[4], WENA[4]);
  buf B155(WENA_[5], WENA[5]);
  buf B156(WENA_[6], WENA[6]);
  buf B157(WENA_[7], WENA[7]);
  buf B158(WENA_[8], WENA[8]);
  buf B159(WENA_[9], WENA[9]);
  buf B160(WENA_[10], WENA[10]);
  buf B161(WENA_[11], WENA[11]);
  buf B162(WENA_[12], WENA[12]);
  buf B163(WENA_[13], WENA[13]);
  buf B164(WENA_[14], WENA[14]);
  buf B165(WENA_[15], WENA[15]);
  buf B166(WENA_[16], WENA[16]);
  buf B167(WENA_[17], WENA[17]);
  buf B168(WENA_[18], WENA[18]);
  buf B169(WENA_[19], WENA[19]);
  buf B170(WENA_[20], WENA[20]);
  buf B171(WENA_[21], WENA[21]);
  buf B172(WENA_[22], WENA[22]);
  buf B173(WENA_[23], WENA[23]);
  buf B174(WENA_[24], WENA[24]);
  buf B175(WENA_[25], WENA[25]);
  buf B176(WENA_[26], WENA[26]);
  buf B177(WENA_[27], WENA[27]);
  buf B178(WENA_[28], WENA[28]);
  buf B179(WENA_[29], WENA[29]);
  buf B180(WENA_[30], WENA[30]);
  buf B181(WENA_[31], WENA[31]);
  buf B182(AA_[0], AA[0]);
  buf B183(AA_[1], AA[1]);
  buf B184(AA_[2], AA[2]);
  buf B185(AA_[3], AA[3]);
  ...
  buf B196(DA_[8], DA[8]);
  buf B197(DA_[9], DA[9]);
  buf B198(DA_[10], DA[10]);
  buf B199(DA_[11], DA[11]);
  buf B200(DA_[12], DA[12]);
  buf B201(DA_[13], DA[13]);
  buf B202(DA_[14], DA[14]);
  buf B203(DA_[15], DA[15]);
  buf B204(DA_[16], DA[16]);
  buf B205(DA_[17], DA[17]);
  buf B206(DA_[18], DA[18]);
  buf B207(DA_[19], DA[19]);
  buf B208(DA_[20], DA[20]);
  buf B209(DA_[21], DA[21]);
  buf B210(DA_[22], DA[22]);
  buf B211(DA_[23], DA[23]);
  buf B212(DA_[24], DA[24]);
  buf B213(DA_[25], DA[25]);
  buf B214(DA_[26], DA[26]);
  buf B215(DA_[27], DA[27]);
  buf B216(DA_[28], DA[28]);
  buf B217(DA_[29], DA[29]);
  buf B218(DA_[30], DA[30]);
  buf B219(DA_[31], DA[31]);
  buf B220(CLKB_, CLKB);
  buf B221(CENB_, CENB);
  buf B222(WENB_[0], WENB[0]);
  buf B223(WENB_[1], WENB[1]);
  buf B224(WENB_[2], WENB[2]);
  buf B225(WENB_[3], WENB[3]);
  buf B226(WENB_[4], WENB[4]);
  buf B227(WENB_[5], WENB[5]);
  buf B228(WENB_[6], WENB[6]);
  buf B229(WENB_[7], WENB[7]);
  buf B230(WENB_[8], WENB[8]);
  buf B231(WENB_[9], WENB[9]);
  buf B232(WENB_[10], WENB[10]);
  buf B233(WENB_[11], WENB[11]);
  buf B234(WENB_[12], WENB[12]);
  buf B235(WENB_[13], WENB[13]);
  buf B236(WENB_[14], WENB[14]);
  buf B237(WENB_[15], WENB[15]);
  buf B238(WENB_[16], WENB[16]);
  buf B239(WENB_[17], WENB[17]);
  buf B240(WENB_[18], WENB[18]);
  buf B241(WENB_[19], WENB[19]);
  buf B242(WENB_[20], WENB[20]);
  buf B243(WENB_[21], WENB[21]);
  buf B244(WENB_[22], WENB[22]);
  buf B245(WENB_[23], WENB[23]);
  buf B246(WENB_[24], WENB[24]);
  buf B247(WENB_[25], WENB[25]);
  buf B248(WENB_[26], WENB[26]);
  buf B249(WENB_[27], WENB[27]);
  buf B250(WENB_[28], WENB[28]);
  buf B251(WENB_[29], WENB[29]);
  buf B252(WENB_[30], WENB[30]);
  buf B253(WENB_[31], WENB[31]);
  buf B254(AB_[0], AB[0]);
  buf B255(AB_[1], AB[1]);
  buf B256(AB_[2], AB[2]);
  buf B257(AB_[3], AB[3]);
  ...
  buf B268(DB_[8], DB[8]);
  buf B269(DB_[9], DB[9]);
  buf B270(DB_[10], DB[10]);
  buf B271(DB_[11], DB[11]);
  buf B272(DB_[12], DB[12]);
  buf B273(DB_[13], DB[13]);
  buf B274(DB_[14], DB[14]);
  buf B275(DB_[15], DB[15]);
  buf B276(DB_[16], DB[16]);
  buf B277(DB_[17], DB[17]);
  buf B278(DB_[18], DB[18]);
  buf B279(DB_[19], DB[19]);
  buf B280(DB_[20], DB[20]);
  buf B281(DB_[21], DB[21]);
  buf B282(DB_[22], DB[22]);
  buf B283(DB_[23], DB[23]);
  buf B284(DB_[24], DB[24]);
  buf B285(DB_[25], DB[25]);
  buf B286(DB_[26], DB[26]);
  buf B287(DB_[27], DB[27]);
  buf B288(DB_[28], DB[28]);
  buf B289(DB_[29], DB[29]);
  buf B290(DB_[30], DB[30]);
  buf B291(DB_[31], DB[31]);
  buf B292(EMAA_[0], EMAA[0]);
  buf B293(EMAA_[1], EMAA[1]);
  buf B294(EMAA_[2], EMAA[2]);
  buf B295(EMAWA_[0], EMAWA[0]);
  buf B296(EMAWA_[1], EMAWA[1]);
  buf B297(EMASA_, EMASA);
  buf B298(EMAB_[0], EMAB[0]);
  buf B299(EMAB_[1], EMAB[1]);
  buf B300(EMAB_[2], EMAB[2]);
  buf B301(EMAWB_[0], EMAWB[0]);
  buf B302(EMAWB_[1], EMAWB[1]);
  buf B303(EMASB_, EMASB);
  buf B304(TENA_, TENA);
  buf B305(TCENA_, TCENA);
  buf B306(TWENA_[0], TWENA[0]);
  buf B307(TWENA_[1], TWENA[1]);
  buf B308(TWENA_[2], TWENA[2]);
  buf B309(TWENA_[3], TWENA[3]);
  buf B310(TWENA_[4], TWENA[4]);
  buf B311(TWENA_[5], TWENA[5]);
  buf B312(TWENA_[6], TWENA[6]);
  buf B313(TWENA_[7], TWENA[7]);
  buf B314(TWENA_[8], TWENA[8]);
  buf B315(TWENA_[9], TWENA[9]);
  buf B316(TWENA_[10], TWENA[10]);
  buf B317(TWENA_[11], TWENA[11]);
  buf B318(TWENA_[12], TWENA[12]);
  buf B319(TWENA_[13], TWENA[13]);
  buf B320(TWENA_[14], TWENA[14]);
  buf B321(TWENA_[15], TWENA[15]);
  buf B322(TWENA_[16], TWENA[16]);
  buf B323(TWENA_[17], TWENA[17]);
  buf B324(TWENA_[18], TWENA[18]);
  buf B325(TWENA_[19], TWENA[19]);
  buf B326(TWENA_[20], TWENA[20]);
  buf B327(TWENA_[21], TWENA[21]);
  buf B328(TWENA_[22], TWENA[22]);
  buf B329(TWENA_[23], TWENA[23]);
  buf B330(TWENA_[24], TWENA[24]);
  buf B331(TWENA_[25], TWENA[25]);
  buf B332(TWENA_[26], TWENA[26]);
  buf B333(TWENA_[27], TWENA[27]);
  buf B334(TWENA_[28], TWENA[28]);
  buf B335(TWENA_[29], TWENA[29]);
  buf B336(TWENA_[30], TWENA[30]);
  buf B337(TWENA_[31], TWENA[31]);
  buf B338(TAA_[0], TAA[0]);
  buf B339(TAA_[1], TAA[1]);
  buf B340(TAA_[2], TAA[2]);
  buf B341(TAA_[3], TAA[3]);
  buf B342(TAA_[4], TAA[4]);
  buf B343(TAA_[5], TAA[5]);
  buf B344(TDA_[0], TDA[0]);
  buf B345(TDA_[1], TDA[1]);
  buf B346(TDA_[2], TDA[2]);
  buf B347(TDA_[3], TDA[3]);
  buf B348(TDA_[4], TDA[4]);
  buf B349(TDA_[5], TDA[5]);
  buf B350(TDA_[6], TDA[6]);
  buf B351(TDA_[7], TDA[7]);
  buf B352(TDA_[8], TDA[8]);
  buf B353(TDA_[9], TDA[9]);
  buf B354(TDA_[10], TDA[10]);
  buf B355(TDA_[11], TDA[11]);
  buf B356(TDA_[12], TDA[12]);
  buf B357(TDA_[13], TDA[13]);
  buf B358(TDA_[14], TDA[14]);
  buf B359(TDA_[15], TDA[15]);
  buf B360(TDA_[16], TDA[16]);
  buf B361(TDA_[17], TDA[17]);
  buf B362(TDA_[18], TDA[18]);
  buf B363(TDA_[19], TDA[19]);
  buf B364(TDA_[20], TDA[20]);
  buf B365(TDA_[21], TDA[21]);
  buf B366(TDA_[22], TDA[22]);
  buf B367(TDA_[23], TDA[23]);
  buf B368(TDA_[24], TDA[24]);
  buf B369(TDA_[25], TDA[25]);
  buf B370(TDA_[26], TDA[26]);
  buf B371(TDA_[27], TDA[27]);
  buf B372(TDA_[28], TDA[28]);
  buf B373(TDA_[29], TDA[29]);
  buf B374(TDA_[30], TDA[30]);
  buf B375(TDA_[31], TDA[31]);
  buf B376(TENB_, TENB);
  buf B377(TCENB_, TCENB);
  buf B378(TWENB_[0], TWENB[0]);
  buf B379(TWENB_[1], TWENB[1]);
  buf B380(TWENB_[2], TWENB[2]);
  buf B381(TWENB_[3], TWENB[3]);
  buf B382(TWENB_[4], TWENB[4]);
  buf B383(TWENB_[5], TWENB[5]);
  buf B384(TWENB_[6], TWENB[6]);
  buf B385(TWENB_[7], TWENB[7]);
  buf B386(TWENB_[8], TWENB[8]);
  buf B387(TWENB_[9], TWENB[9]);
  buf B388(TWENB_[10], TWENB[10]);
  buf B389(TWENB_[11], TWENB[11]);
  buf B390(TWENB_[12], TWENB[12]);
  buf B391(TWENB_[13], TWENB[13]);
  buf B392(TWENB_[14], TWENB[14]);
  buf B393(TWENB_[15], TWENB[15]);
  buf B394(TWENB_[16], TWENB[16]);
  buf B395(TWENB_[17], TWENB[17]);
  buf B396(TWENB_[18], TWENB[18]);
  buf B397(TWENB_[19], TWENB[19]);
  buf B398(TWENB_[20], TWENB[20]);
  buf B399(TWENB_[21], TWENB[21]);
  buf B400(TWENB_[22], TWENB[22]);
  buf B401(TWENB_[23], TWENB[23]);
  buf B402(TWENB_[24], TWENB[24]);
  buf B403(TWENB_[25], TWENB[25]);
  buf B404(TWENB_[26], TWENB[26]);
  buf B405(TWENB_[27], TWENB[27]);
  buf B406(TWENB_[28], TWENB[28]);
  buf B407(TWENB_[29], TWENB[29]);
  buf B408(TWENB_[30], TWENB[30]);
  buf B409(TWENB_[31], TWENB[31]);
  buf B410(TAB_[0], TAB[0]);
  buf B411(TAB_[1], TAB[1]);
  buf B412(TAB_[2], TAB[2]);
  buf B413(TAB_[3], TAB[3]);
  buf B414(TAB_[4], TAB[4]);
  buf B415(TAB_[5], TAB[5]);
  buf B416(TDB_[0], TDB[0]);
  buf B417(TDB_[1], TDB[1]);
  buf B418(TDB_[2], TDB[2]);
  buf B419(TDB_[3], TDB[3]);
  buf B420(TDB_[4], TDB[4]);
  buf B421(TDB_[5], TDB[5]);
  buf B422(TDB_[6], TDB[6]);
  buf B423(TDB_[7], TDB[7]);
  buf B424(TDB_[8], TDB[8]);
  buf B425(TDB_[9], TDB[9]);
  buf B426(TDB_[10], TDB[10]);
  buf B427(TDB_[11], TDB[11]);
  buf B428(TDB_[12], TDB[12]);
  buf B429(TDB_[13], TDB[13]);
  buf B430(TDB_[14], TDB[14]);
  buf B431(TDB_[15], TDB[15]);
  buf B432(TDB_[16], TDB[16]);
  buf B433(TDB_[17], TDB[17]);
  buf B434(TDB_[18], TDB[18]);
  buf B435(TDB_[19], TDB[19]);
  buf B436(TDB_[20], TDB[20]);
  buf B437(TDB_[21], TDB[21]);
  buf B438(TDB_[22], TDB[22]);
  buf B439(TDB_[23], TDB[23]);
  buf B440(TDB_[24], TDB[24]);
  buf B441(TDB_[25], TDB[25]);
  buf B442(TDB_[26], TDB[26]);
  buf B443(TDB_[27], TDB[27]);
  buf B444(TDB_[28], TDB[28]);
  buf B445(TDB_[29], TDB[29]);
  buf B446(TDB_[30], TDB[30]);
  buf B447(TDB_[31], TDB[31]);
  buf B448(GWENA_, GWENA);
  buf B449(GWENB_, GWENB);
  buf B450(TGWENA_, TGWENA);
  buf B451(TGWENB_, TGWENB);
  buf B452(RET1N_, RET1N);
  buf B453(SIA_[0], SIA[0]);
  buf B454(SIA_[1], SIA[1]);
  buf B455(SEA_, SEA);
  buf B456(DFTRAMBYP_, DFTRAMBYP);
  buf B457(SIB_[0], SIB[0]);
  buf B458(SIB_[1], SIB[1]);
  buf B459(SEB_, SEB);
  buf B460(COLLDISN_, COLLDISN);

  assign CENYA_ = (RET1N_ | pre_charge_st) ? (DFTRAMBYP_ & (TENA_ ? CENA_ : TCENA_)) : 1'bx;
  assign WENYA_ = (RET1N_ | pre_charge_st) ? ({32{DFTRAMBYP_}} & (TENA_ ? WENA_ : TWENA_)) : {32{1'bx}};
  ...
  assign WENYB_ = (RET1N_ | pre_charge_st) ? ({32{DFTRAMBYP_}} & (TENB_ ? WENB_ : TWENB_)) : {32{1'bx}};
...
`endif
  ...
  always @ globalNotifier0 begin
    ...
    if (clk0_int === 1'bx || CENA_int === 1'bx) begin
     ...
     && row_contention(TENB_ ? AB_ : TAB_, AA_int, ({32{GWENA_int}}|WENA_int), TENB_ ? ({32{GWENB_}}|WENB_) : ({32{TGWENB_}}|TWENB_))) begin
      cont_flag0_int = 1'b0;
        if (col_contention(AA_int, AB_int)) begin
          COL_CC = 1;
        end
...
`endif
     ...
     || COLLDISN_int === 1'bx) && row_contention(TENB_ ? AB_ : TAB_, AA_int, ({32{GWENA_int}}|WENA_int), TENB_ ? ({32{GWENB_}}|WENB_) : ({32{TGWENB_}}|TWENB_))) 
     begin
      cont_flag0_int = 1'b0;
          ROW_CC = 1;
          READ_READ_1 = 0;
...
`endif
  ...
  always @ globalNotifier1 begin
    ...
    if (clk1_int === 1'bx || CENB_int === 1'bx) begin
     ...
     && row_contention(TENA_ ? AA_ : TAA_, AB_int, ({32{GWENB_int}}|WENB_int), TENA_ ? ({32{GWENA_}}|WENA_) : ({32{TGWENA_}}|TWENA_))) begin
      cont_flag1_int = 1'b0;
        if (col_contention(AA_int, AB_int)) begin
          COL_CC = 1;
        end
...
`endif
     ...
     || COLLDISN_int === 1'bx) && row_contention(TENA_ ? AA_ : TAA_, AB_int, ({32{GWENB_int}}|WENB_int), TENA_ ? ({32{GWENA_}}|WENA_) : ({32{TGWENA_}}|TWENA_))) 
     begin
      cont_flag1_int = 1'b0;
          ROW_CC = 1;
          READ_READ_1 = 0;
...
`endif
  ...
  begin
   ...
   wire contA_flag = (CENA_int !== 1'b1 && ((TENB_ ? CENB_ : TCENB_) !== 1'b1)) && ((COLLDISN_int === 1'b1 && is_contention(TENB_ ? AB_ : TAB_, AA_int, TENB_ ? ({32{GWENB_}}|WENB_) : ({32{TGWENB_}}|TWENB_), ({32{GWENA_int}}|WENA_int))) ||
              ((COLLDISN_int === 1'b0 || COLLDISN_int === 1'bx) && row_contention(TENB_ ? AB_ : TAB_, AA_int, TENB_ ? ({32{GWENB_}}|WENB_) : ({32{TGWENB_}}|TWENB_), ({32{GWENA_int}}|WENA_int))));
   wire contB_flag = (CENB_int !== 1'b1 && ((TENA_ ? CENA_ : TCENA_) !== 1'b1)) && ((COLLDISN_int === 1'b1 && is_contention(TENA_ ? AA_ : TAA_, AB_int, TENA_ ? ({32{GWENA_}}|WENA_) : ({32{TGWENA_}}|TWENA_), ({32{GWENB_int}}|WENB_int))) ||
              ((COLLDISN_int === 1'b0 || COLLDISN_int === 1'bx) && row_contention(TENA_ ? AA_ : TAA_, AB_int, TENA_ ? ({32{GWENA_}}|WENA_) : ({32{TGWENA_}}|TWENA_), ({32{GWENB_int}}|WENB_int))));
...
`endif
`timescale 1ns/1ps
module arm28hkcpdpsram64x32m4_error_injection (Q_out, Q_in, CLK, A, CEN, DFTRAMBYP, SE, WEN, GWEN);
   output [31:0] Q_out;
   input [31:0] Q_in;
   input CLK;
   input [5:0] A;
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.SMARCHCHKBVCD_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(6'd1,5'd6,2'd1,2'd0);
   `endif
end
   task add_fault;
