...
/*
    ...
    Filename   : smic28sp8192x8m8.v
    IP code    : S28HKCPHDSP
    Version    : V0P2
    CreateDate : Jul 11, 2025 12:01:08 PM

    Verilog Model for High Speed Single-Port SRAM
    SMIC 28 SFE Process

    Configuration: -instname smic28sp8192x8m8 
                   -words 8192 -bits 8 -mux 8 -bank 4
...
`endif
  ...
  // PARAMETERS
  //////////////////////////////////////////////
  parameter WORDS        = 8192;  // words
  parameter A_WIDTH      = 13;  // address bits
  parameter D_WIDTH      = 8;  // data bits
  parameter MUX          = 8;  // mux
...
`endif
  ...
  begin
      ...
      data_new = {7'b0000000, data_real[7], 7'b0000000, data_real[6], 
      ...
      pos = 64'b0000000100000001000000010000000100000001000000010000000100000001 << col_address;
...
`endif
  assign nf_inputs_z = (CLK_buf === 1'bz) || (CEN_buf === 1'bz) || (WEN_buf === 1'bz) || (A_buf[0] === 1'bz) || (A_buf[1] === 1'bz) || 
                ...
                (A_buf[12] === 1'bz) || (D_buf[0] === 1'bz) || (D_buf[1] === 1'bz) || (D_buf[2] === 1'bz) || (D_buf[3] === 1'bz) || 
                (D_buf[4] === 1'bz) || (D_buf[5] === 1'bz) || (D_buf[6] === 1'bz) || (D_buf[7] === 1'bz) || (BWEN_buf[0] === 1'bz) || 
                (BWEN_buf[1] === 1'bz) || (BWEN_buf[2] === 1'bz) || (BWEN_buf[3] === 1'bz) || (BWEN_buf[4] === 1'bz) || (BWEN_buf[5] === 1'bz) || 
                (BWEN_buf[6] === 1'bz) || (BWEN_buf[7] === 1'bz) || 1'b0;
...
`else
  ...
  always @(TM_noti) begin
    ...
    assign BWEN_tmp[7] = ^BWEN[7];`endif
endmodule
`endcelldefine
