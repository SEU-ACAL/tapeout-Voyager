    ...
    for (k=0; k<COL_GROUP_NUM; k=k+1) begin
        always @(posedge clk or negedge rstn) begin
            if (!rstn) begin
                ...
                psum_temp[(k+1)*(PSUM_8_W + 7)-1 -: (PSUM_8_W + 7)] <= $signed(psum_temp[(k+1)*(PSUM_8_W + 7)-1 -: (PSUM_8_W + 7)]) + $signed({{7{PSUM_8[(k+1)*PSUM_8_W-1]}},PSUM_8[(k+1)*PSUM_8_W-1 -: PSUM_8_W]} << bitcount);
            end
        end
    end
endgenerate
