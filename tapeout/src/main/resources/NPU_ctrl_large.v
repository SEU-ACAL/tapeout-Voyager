...
module NPU_ctrl_large #(
    ...
    generate
        for (i = 0; i < 32; i = i + 1) begin : gen_obl_add
            wire signed [31:0] sum_in_1 = outlier_sum_L[i*32 +: 32];
            wire signed [31:0] sum_in_2 = data_out_L[i*32 +: 32];
            wire signed [31:0] sum_res = (sum_in_1 <<< 8) + sum_in_2;

            assign obl_npu_store_data[i*32 +: 32] = sum_res;
        end
    endgenerate
