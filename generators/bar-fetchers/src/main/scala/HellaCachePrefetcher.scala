...
class HellaCachePrefetchWrapperModule(pP: CanInstantiatePrefetcher, outer: HellaCachePrefetchWrapper) extends HellaCacheModule(outer) with MemoryOpConstants{
  ...
  prefetcher.io.stride.bits.pc := io.cpu.req.bits.pc
  prefetcher.io.stride.bits.addr := io.cpu.req.bits.addr
  prefetcher.io.stride.valid := io.cpu.req.fire && !core_prefetch
  prefetcher.io.stride.bits.kill := io.cpu.s1_kill
  prefetcher.io.stride.bits.read := isRead(io.cpu.req.bits.cmd)
