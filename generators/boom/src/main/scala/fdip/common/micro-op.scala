...
import boom.fdip.exu.FUConstants
  ...
  with freechips.rocketchip.rocket.constants.ScalarOpConstants
{
  ...
  val ppred            = UInt((log2Ceil(ftqSz)+1).W)
  ...
  def allocate_brtag   = (is_br && !is_sfb) || is_jalr || is_jal
  ...
  def unsafe           = uses_ldq || (uses_stq && !is_fence) || is_br || is_jalr || is_jal
