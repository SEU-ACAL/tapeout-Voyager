...
package boom.fdip.common.constants
...
//val uopSRET = 40.U(UOPC_SZ.W)
  ...
  def NullMicroOp()(implicit p: Parameters): boom.fdip.common.MicroOp = {
    val uop = Wire(new boom.fdip.common.MicroOp)
    ...
    val cs = Wire(new boom.fdip.common.CtrlSignals())
...
trait RISCVConstants
{
  ...
  def GetCfiType(inst: UInt)(implicit p: Parameters): UInt = {
    val bdecode = Module(new boom.fdip.exu.BranchDecode)
