...
import boom.fdip.lsu.{BoomNonBlockingDCache, LSU, LSUCoreIO}
import boom.fdip.common.{BoomTileParams, MicroOp, BoomCoreParams, BoomModule}
...
class BoomLSUShim(implicit p: Parameters) extends BoomModule()(p)
  ...
  io.lsu.brupdate.b1 := (0.U).asTypeOf(new boom.fdip.exu.BrUpdateMasks)
