...
import boom.fdip.exu.{BrUpdateInfo, Exception, FuncUnitResp, CommitSignals, ExeUnitResp}
import boom.fdip.util.{BoolToChar, AgePriorityEncoder, IsKilledByBranch, GetNewBrMask, WrapInc, IsOlder, UpdateBrMask}
...
class LSUExeIO(implicit p: Parameters) extends BoomBundle()(p)
{
  ...
  val iresp    = new DecoupledIO(new boom.fdip.exu.ExeUnitResp(xLen))
  val fresp    = new DecoupledIO(new boom.fdip.exu.ExeUnitResp(xLen+1)) // TODO: Should this be fLen?
}
