...
import boom.fdip.ifu.{GetPCFromFtqIO}
import boom.fdip.util.{ImmGen, IsKilledByBranch, BranchKillableQueue, BoomCoreStringPrefix}
  ...
  )(implicit p: Parameters) extends BoomModule
{
  ...
  val io = IO(new Bundle {
    ...
    val lsu_io = if (hasMem) Flipped(new boom.fdip.lsu.LSUExeIO) else null
