...
class ALUUnit(isJmpUnit: Boolean = false, numStages: Int = 1, dataWidth: Int)(implicit p: Parameters)
  extends PipelinedFunctionalUnit(
    ...
    dataWidth = dataWidth,
    isJmpUnit = isJmpUnit)
  with boom.fdip.ifu.HasBoomFrontendParameters
{
  ...
  // brinfo :=DontCare
  when (is_br || is_jalr || is_jal) {
    if (!isJmpUnit) {
      assert (pc_sel =/= PC_JALR)
    }
    when (pc_sel === PC_PLUS4) {
      mispredict := uop.taken
      // assert(!(io.get_ftq_pc.next_val && (io.get_ftq_pc.next_pc =/= (brj_npc.asUInt))),
      //   "PC_PLUS4 but next_pc doesn't match fallthrough.")
    }
  ...
  if (isJmpUnit) {
    ...
    val brj_block_pc  = Wire(UInt(vaddrBitsExtended.W))//AlignPCToBoundary(io.get_ftq_pc.pc, icBlockBytes)
    brj_block_pc := AlignPCToBoundary(io.get_ftq_pc.pc, icBlockBytes)
    val brj_uop_pc    = (brj_block_pc | uop.pc_lob) 
    val brj_npc       = brj_uop_pc + Mux(uop.is_rvc || uop.edge_inst, 2.U, 4.U)
    val jal_br_target = Wire(UInt(vaddrBitsExtended.W))


    jal_br_target := (brj_uop_pc.asSInt + target_offset +
    ...
    val cfi_idx = ((uop.pc_lob ))(log2Ceil(fetchWidth),1)
    ...
    }.elsewhen(pc_sel === PC_BRJMP) {
      mispredict := !io.get_ftq_pc.next_val ||
                    (io.get_ftq_pc.next_pc =/= jal_br_target) ||
                    !io.get_ftq_pc.entry.cfi_idx.valid ||
                    (io.get_ftq_pc.entry.cfi_idx.bits =/= cfi_idx)
    }
  } 
  ...
  brinfo.valid          := is_br || is_jalr || is_jal
  brinfo.mispredict     := mispredict
  brinfo.uop            := uop
  brinfo.cfi_type       := Mux(is_jalr, CFI_JALR,
                          Mux(is_br  , CFI_BR, 
                          Mux(is_jal , CFI_JAL, CFI_X)))
  brinfo.taken          := is_taken
  brinfo.pc_sel         := pc_sel
  brinfo.is_call        := (is_jal || is_jalr) && uop.ldst===1.U
  brinfo.is_ret         := is_jalr && uop.lrs1 === 5.U && !brinfo.is_call
