...
import boom.fdip.ifu.{GlobalHistory, HasBoomFrontendParameters}
import boom.fdip.exu.FUConstants._
  ...
  with HasBoomFrontendParameters // TODO: Don't add this trait
{
  ...
  val exe_units = new boom.fdip.exu.ExecutionUnits(fpu=false)
  ...
  b2.is_call    := oldest_mispredict.is_call
  b2.is_ret     := oldest_mispredict.is_ret
...
//      ("I$ blocked",                        () => icache_blocked),
  ...
  val jmp_pc_req  = Wire(Decoupled(UInt((log2Ceil(ftqSz)+1).W)))
  val xcpt_pc_req = Wire(Decoupled(UInt((log2Ceil(ftqSz)+1).W)))
  val flush_pc_req = Wire(Decoupled(UInt((log2Ceil(ftqSz)+1).W)))

  val ftq_arb = Module(new Arbiter(UInt((log2Ceil(ftqSz)+1).W), 3))
  dontTouch(jmp_pc_req)
  ...
  val dis_stalls = WireInit(VecInit(dis_hazards.scanLeft(false.B) ((s,h) => s || h).takeRight(coreWidth)))
  dis_fire := dis_valids zip dis_stalls map {case (v,s) => v && !s}
  dis_ready := !dis_stalls.last

  dontTouch(dis_stalls)
  //-------------------------------------------------------------
  ...
  csr.io.pc        := (boom.fdip.util.AlignPCToBoundary(io.ifu.get_pc(0).com_pc, icBlockBytes)
 ...
 */ 
  ...
  val Icache_miss     = RegInit(0.U(64.W))
  val Icache_prefetch = RegInit(0.U(64.W))
  val ftq_redirect_conflict = RegInit(0.U(64.W))
  val ftq_meta_conflict     = RegInit(0.U(64.W))
  ...
  Icache_miss        := Mux(perf_ctrl===0.U,0.U,Mux(perf_ctrl===2.U,Icache_miss        ,Mux(perf_ctrl===1.U,Icache_miss   +io.ifu.perf.icache_miss.asUInt,Icache_miss   )))
  Icache_prefetch    := Mux(perf_ctrl===0.U,0.U,Mux(perf_ctrl===2.U,Icache_prefetch        ,Mux(perf_ctrl===1.U,Icache_prefetch   +io.ifu.perf.icache_prefetch.asUInt,Icache_prefetch   )))
  ftq_redirect_conflict := Mux(perf_ctrl===0.U,0.U,Mux(perf_ctrl===2.U,ftq_redirect_conflict        ,Mux(perf_ctrl===1.U,ftq_redirect_conflict   + io.ifu.perf.ftq_redirect_conflict.asUInt,ftq_redirect_conflict   )))
  ftq_meta_conflict     := Mux(perf_ctrl===0.U,0.U,Mux(perf_ctrl===2.U,ftq_meta_conflict        ,Mux(perf_ctrl===1.U,ftq_meta_conflict   + io.ifu.perf.ftq_meta_conflict.asUInt,ftq_meta_conflict   )))
  io.debug_perf_val:=  MuxLookup(perf_ctrl, 0.U )(Seq(
                        ...
                        6.U -> Icache_miss,
                        7.U -> Icache_prefetch,
                        8.U -> ftq_redirect_conflict,
                        9.U -> Br_commit,
                        10.U -> Jal_commit,
                        11.U -> Jalr_commit,
                        12.U -> Int_commit,
                        13.U -> ftq_meta_conflict,
                        14.U -> Mul_commit,
                        15.U -> Total_mispred,
                        16.U -> Br_mispred,
                        17.U -> Cycle
  ...
  if (trace) {
    for (w <- 0 until coreWidth) {
      ...
      io.trace.insns(w).iaddr      := RegNext(rob.io.commit.uops(w).debug_pc)
