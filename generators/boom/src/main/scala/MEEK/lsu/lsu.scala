...
import boom.meek.exu.{BrUpdateInfo, Exception, FuncUnitResp, CommitSignals, ExeUnitResp}
import boom.meek.util.{BoolToChar, AgePriorityEncoder, IsKilledByBranch, GetNewBrMask, WrapInc, IsOlder, UpdateBrMask}
...
class LSUExeIO(implicit p: Parameters) extends BoomBundle()(p)
{
  ...
  val iresp    = new DecoupledIO(new boom.meek.exu.ExeUnitResp(xLen))
  val fresp    = new DecoupledIO(new boom.meek.exu.ExeUnitResp(xLen+1)) // TODO: Should this be fLen?
}
...
//===== GuardianCouncil Function: Start ====//
  val stq_debug = Output(Valid(new STQEntry))
//===== GuardianCouncil Function: End ====//
}
...
//===== GuardianCouncil Function: Start ====//
  ...
  io.core.stq_debug := stq(stq_commit_head)
  //===== GuardianCouncil Function: End ====//
}
