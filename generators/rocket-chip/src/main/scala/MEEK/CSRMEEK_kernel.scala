...
trait IO_for_kernel_kernel extends CoreBundle {
  ...
  //debug
  val read_sstatus_sdw     = Output(UInt())
  val read_mstatus_sdw    = Output(UInt())
  //===== GuardianCouncil Function: End ====//
}

class CSRFileIOMEEK_kernel(hasBeu: Boolean)(implicit p: Parameters) extends CoreBundle
    with HasCoreParameters
    with IO_for_kernel_kernel {
...
class CSRFileMEEK_kernel(
  perfEventSets: freechips.rocketchip.meek.EventSets = new freechips.rocketchip.meek.EventSets(Seq()),
  customCSRs: Seq[CustomCSR] = Nil,
  roccCSRs: Seq[CustomCSR] = Nil,
  hasBeu: Boolean = false)(implicit p: Parameters)
    extends CoreModule()(p)
    with HasCoreParameters {
  val io = IO(new CSRFileIOMEEK_kernel(hasBeu) {
    val customCSRs = Vec(CSRFileMEEK_kernel.this.customCSRs.size, new CustomCSRIO)
    val roccCSRs = Vec(CSRFileMEEK_kernel.this.roccCSRs.size, new CustomCSRIO)
  })
  ...
  /* MEEK Kernel start */
  val reg_fflags_shadow = if(tileId != 0) Some(Reg(UInt(5.W))) else None
  val reg_frm_shadow = if(tileId != 0) Some(Reg(UInt(3.W))) else None
  val reg_shadow = if(tileId != 0) Some(Reg(Vec(CSRshadows.CSRsize, UInt(xLen.W)))) else None
  ...
  val read_sstatus_shadow = WireDefault(0.U.asTypeOf(new MStatus))
  val read_sstatus_real = Mux(io.checker_priv_mode, read_sstatus_shadow.asUInt(xLen-1,0), (read_sstatus.asUInt)(xLen-1,0))
  if (usingSupervisor) {
    val read_sie = Mux(io.checker_priv_mode, reg_shadow.getOrElse(VecInit(Seq.fill(CSRshadows.CSRsize)(1.U)))(CSRshadowsindex.mie) & sie_mask, reg_mie & sie_mask)
    val read_sip = Mux(io.checker_priv_mode, read_mip_shadow & sie_mask, read_mip & sie_mask)
    ...
    read_sstatus_shadow.sd := reg_shadow.getOrElse(VecInit(Seq.fill(CSRshadows.CSRsize)(1.U)))(CSRshadowsindex.mstatus).asTypeOf(new MStatus).sd
    read_sstatus_shadow.uxl := reg_shadow.getOrElse(VecInit(Seq.fill(CSRshadows.CSRsize)(1.U)))(CSRshadowsindex.mstatus).asTypeOf(new MStatus).uxl
    read_sstatus_shadow.sd_rv32 := reg_shadow.getOrElse(VecInit(Seq.fill(CSRshadows.CSRsize)(1.U)))(CSRshadowsindex.mstatus).asTypeOf(new MStatus).sd_rv32
    read_sstatus_shadow.mxr := reg_shadow.getOrElse(VecInit(Seq.fill(CSRshadows.CSRsize)(1.U)))(CSRshadowsindex.mstatus).asTypeOf(new MStatus).mxr
    read_sstatus_shadow.sum := reg_shadow.getOrElse(VecInit(Seq.fill(CSRshadows.CSRsize)(1.U)))(CSRshadowsindex.mstatus).asTypeOf(new MStatus).sum
    read_sstatus_shadow.xs := reg_shadow.getOrElse(VecInit(Seq.fill(CSRshadows.CSRsize)(1.U)))(CSRshadowsindex.mstatus).asTypeOf(new MStatus).xs
    read_sstatus_shadow.fs := reg_shadow.getOrElse(VecInit(Seq.fill(CSRshadows.CSRsize)(1.U)))(CSRshadowsindex.mstatus).asTypeOf(new MStatus).fs
    read_sstatus_shadow.vs := reg_shadow.getOrElse(VecInit(Seq.fill(CSRshadows.CSRsize)(1.U)))(CSRshadowsindex.mstatus).asTypeOf(new MStatus).vs
    read_sstatus_shadow.spp := reg_shadow.getOrElse(VecInit(Seq.fill(CSRshadows.CSRsize)(1.U)))(CSRshadowsindex.mstatus).asTypeOf(new MStatus).spp
    read_sstatus_shadow.spie := reg_shadow.getOrElse(VecInit(Seq.fill(CSRshadows.CSRsize)(1.U)))(CSRshadowsindex.mstatus).asTypeOf(new MStatus).spie
    read_sstatus_shadow.sie := reg_shadow.getOrElse(VecInit(Seq.fill(CSRshadows.CSRsize)(1.U)))(CSRshadowsindex.mstatus).asTypeOf(new MStatus).sie
  ...
  io.evec := Mux(exception, tvec, Mux(io.check_exception, io.check_tvec, tvec))
  io.ptbr := reg_satp
  io.hgatp := reg_hgatp
  io.vsatp := reg_vsatp
  io.eret := insn_call || insn_break || insn_ret
  io.eret_nocall := insn_break || insn_ret
  ...
  /* Kernel End */
  exception := insn_call || insn_break || io.exception
...
/* Kernel Start */
  ...
  //   reg_mstatus.mie := false.B
  //   new_prv := PRV.M.U
  // }.elsewhen(io.check_exception && io.check_priv === PRV.S.U){ //只是为了verilator实验，实际boot linux，这里应该切到S-mode
  //   reg_mstatus.v := false.B
  //   reg_hstatus.spvp := Mux(reg_mstatus.v, reg_mstatus.prv(0),reg_hstatus.spvp)
  //   reg_hstatus.gva := io.gva
  //   reg_hstatus.spv := reg_mstatus.v
...
/* Kernel Start */
  val pre_ret_prv = RegInit(0.U(2.W))
  when (insn_ret) {
  ...
  when((io.check_ret_priv < pre_ret_prv) && shadow_status === 1.U && !io.arfs_is_CPS && io.arfs_is_CSR && io.shadow_idx === 7.U){
    val mstatus_wire = Wire(new MStatus())
    val ret_prv = WireInit(UInt(), DontCare)
...
/* Kernel Start */
  ...
  val set_fs_dirty_shadow = if(tileId != 0) Some(WireDefault(io.set_fs_dirty.getOrElse(false.B))) else None
  ...
  when (csr_wen) {
    ...
    /* Kernel Start */
    if (usingFPU) {
      when (decoded_addr(CSRs.fflags) && !(io.checker_mode || io.checker_priv_mode)) { set_fs_dirty := true.B; reg_fflags := wdata }.elsewhen(decoded_addr(CSRs.fflags) && (io.checker_mode || io.checker_priv_mode)) { if(tileId != 0) { set_fs_dirty_shadow.get := true.B; reg_fflags_shadow.get := wdata }}
      when (decoded_addr(CSRs.frm) && !(io.checker_mode || io.checker_priv_mode))    { set_fs_dirty := true.B; reg_frm := wdata }.elsewhen(decoded_addr(CSRs.frm) && (io.checker_mode || io.checker_priv_mode)) { if(tileId != 0) { set_fs_dirty_shadow.get := true.B; reg_frm_shadow.get := wdata }}
      //===== GuardianCouncil Function: Start ====//
  ...
  //===== GuardianCouncil Function: Start ====//
  /* R Features */
  io.read_sstatus_sdw := read_sstatus_real
  io.read_mstatus_sdw := read_mstatus
