...
class CSRFileIOMEEK(hasBeu: Boolean)(implicit p: Parameters) extends CoreBundle
...
class CSRFileMEEK(
  ...
  hasBeu: Boolean = false)(implicit p: Parameters)
    ...
    with HasCoreParameters {
  val io = IO(new CSRFileIOMEEK(hasBeu) {
    val customCSRs = Vec(CSRFileMEEK.this.customCSRs.size, new CustomCSRIO)
    val roccCSRs = Vec(CSRFileMEEK.this.roccCSRs.size, new CustomCSRIO)
  })
  ...
  val reg_fflags_shadow = if(tileParams.tileId != 0) Some(Reg(UInt(5.W))) else None
  val reg_frm_shadow = if(tileParams.tileId != 0) Some(Reg(UInt(3.W))) else None

  val reg_shadow = if(tileParams.tileId != 0) Some(Reg(Vec(CSRshadows.CSRsize, UInt(xLen.W)))) else None
  ...
  val fp_csrs = LinkedHashMap[Int,Bits]() ++
    usingFPU.option(CSRs.fflags -> Mux(io.checker_mode, reg_fflags_shadow.getOrElse(0.U), reg_fflags)) ++
    usingFPU.option(CSRs.frm -> Mux(io.checker_mode, reg_frm_shadow.getOrElse(0.U), reg_frm)) ++
    (usingFPU || usingVector).option(CSRs.fcsr -> Mux(io.checker_mode, read_fcsr, read_fcsr_shadow))
  //===== GuardianCouncil Function: END ====//
...
//===== GuardianCouncil Function: Start ====//
  val set_fs_dirty_shadow = if(tileParams.tileId != 0) Some(WireDefault(io.set_fs_dirty.getOrElse(false.B))) else None
  ...
  when (set_fs_dirty_shadow.getOrElse(false.B)){
    if(tileParams.tileId != 0){
      ...
      reg_shadow.get(CSRshadowsindex.mstatus).asTypeOf(new MStatus).fs := 3.U
    }
  }
  io.fcsr_rm := Mux(!io.checker_mode, reg_frm, reg_frm_shadow.getOrElse(0.U))
  when (io.fcsr_flags.valid && !io.checker_mode) {
    reg_fflags := reg_fflags | io.fcsr_flags.bits    
    set_fs_dirty := true.B
  }.elsewhen(io.fcsr_flags.valid && io.checker_mode){
    if(tileParams.tileId != 0){
...
//===== GuardianCouncil Function: END ====//
  ...
  when (csr_wen) {
    ...
    //===== GuardianCouncil Function: Start ====//
    if (usingFPU) {
      when (decoded_addr(CSRs.fflags) && !io.checker_mode) { set_fs_dirty := true.B; reg_fflags := wdata }.elsewhen(decoded_addr(CSRs.fflags) && io.checker_mode) { if(tileParams.tileId != 0) { set_fs_dirty_shadow.get := true.B; reg_fflags_shadow.get := wdata }}
      when (decoded_addr(CSRs.frm) && !io.checker_mode)    { set_fs_dirty := true.B; reg_frm := wdata }.elsewhen(decoded_addr(CSRs.frm) && io.checker_mode) { if(tileParams.tileId != 0) { set_fs_dirty_shadow.get := true.B; reg_frm_shadow.get := wdata }}
      //===== GuardianCouncil Function: Start ====//
      when (decoded_addr(CSRs.fcsr) && !io.checker_mode) {
        set_fs_dirty := true.B
        reg_fflags := wdata
        reg_frm := wdata >> reg_fflags.getWidth
      }.elsewhen(decoded_addr(CSRs.fcsr) && io.checker_mode){
        if(tileParams.tileId != 0){
          set_fs_dirty_shadow.get := true.B
