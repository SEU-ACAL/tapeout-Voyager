...
class R_ICIO(params: R_ICParams) extends Bundle {
  ...
  // val if_big_complete_ack                        = Output(Vec(params.totalnumber_of_cores-1, Bool()))
...
class R_IC (val params: R_ICParams) extends Module with HasR_ICIO {
  ...
  switch (fsm_state) {
    ...
    is (fsm_sch){ // 010
      ...
      crnt_target                               := crnt_target
      crnt_mask                                 := crnt_mask
      //!ic_status(sch_result).asBool可能是在此时需要检查调度的核心不能被使用
      // assert(io.if_correct_process.asBool&(!ic_status(sch_result).asBool),"sch core is not avaliable")
