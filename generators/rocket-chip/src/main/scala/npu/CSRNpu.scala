...
class CSRNpuDecodeIO(implicit p: Parameters) extends CoreBundle {
...
class CSRFileNpuIO(hasBeu: Boolean)(implicit p: Parameters) extends CoreBundle
  ...
  val decode = Vec(decodeWidth, new CSRNpuDecodeIO)
...
class CSRFileNpu(
  ...
  hasBeu: Boolean = false)(implicit p: Parameters)
    ...
    with HasCoreParameters {
  val io = IO(new CSRFileNpuIO(hasBeu) {
    val customCSRs = Vec(CSRFileNpu.this.customCSRs.size, new CustomCSRIO)
    val roccCSRs = Vec(CSRFileNpu.this.roccCSRs.size, new CustomCSRIO)
  })
