    ...
    with HasICacheFrontend
{
  ...
  def this(params: RocketTileNpuParams, crossing: HierarchicalElementCrossingParamsLike, lookup: LookupByHartIdImpl)(implicit p: Parameters) =
    this(params, crossing.crossingType, lookup, p)

  val intOutwardNode = rocketnpuParams.beuAddr map { _ => IntIdentityNode() }
  ...
  nDCachePorts += 1 /*core */ + (dtim_adapter.isDefined).toInt + rocketnpuParams.core.vector.map(_.useDCache.toInt).getOrElse(0)
  ...
  val vector_unit = rocketnpuParams.core.vector.map(v => LazyModule(v.build(p)))
  ...
  override def makeMasterBoundaryBuffers(crossing: ClockCrossingType)(implicit p: Parameters) = (rocketnpuParams.boundaryBuffers, crossing) match {
  ...
  override def makeSlaveBoundaryBuffers(crossing: ClockCrossingType)(implicit p: Parameters) = (rocketnpuParams.boundaryBuffers, crossing) match {
    case (Some(RocketTileBoundaryBufferParams(true )), _)                   => TLBuffer()
...
class RocketTileNpuModuleImp(outer: RocketTileNpu) extends BaseTileModuleImp(outer)
  ...
  outer.vector_unit foreach { v => if (outer.rocketnpuParams.core.vector.get.useDCache) {
...
trait HasFpuNpuOpt { this: RocketTileNpuModuleImp =>
  ...
  fpuOpt.foreach { fpu =>
    ...
    val nFPUPorts = nRoCCFPUPorts + outer.rocketnpuParams.core.useVector.toInt
    if (nFPUPorts > 0) {
