...
trait HasRocketCoreNpuIO extends HasRocketCoreParameters {
  implicit val p: Parameters
  def nTotalRoCCCSRs: Int
  val io = IO(new CoreBundle()(p) {
    val hartid = Input(UInt(hartIdLen.W))
    val reset_vector = Input(UInt(resetVectorLen.W))
    val interrupts = Input(new CoreInterrupts(tileParams.asInstanceOf[RocketTileNpuParams].beuAddr.isDefined))
...
class RocketNpu(tile: RocketTileNpu)(implicit p: Parameters) extends CoreModule()(p)
  ...
  val csr = Module(new CSRFileNpu(perfEvents, coreParams.customCSRs.decls, tile.roccCSRs.flatten, tile.rocketnpuParams.beuAddr.isDefined))
  ...
  io.dmem.req.bits.pc.get := ex_reg_pc
