...
class id_iss extends Module {  
  ...
  val id_iss_hs = io.id_iss_i.valid & io.id_iss_o.ready
  ...
  io.id_iss_o.bits.config       := uint_dff(id_iss_hs,                       0.U(13.W), io.id_iss_i.bits.config)
...
class iss_ex extends Module {
  ...
  val iss_ex_hs = io.iss_ex_i.valid & io.iss_ex_o.ready
  ...
  io.iss_ex_o.bits.config    := uint_dff(iss_ex_hs,                       0.U(13.W), io.iss_ex_i.bits.config)
