...
class Vec4PE extends Module{
  ...
  val threads = Seq.fill(4)(Module(new VecThread()))
  for(i <- 0 until 4){
    ...
    threads(i).io.in.bits.config    := 0.U(16.W) 
    threads(i).io.in.bits.iteration := 0.U(4.W)
    threads(i).io.in.bits.thread_id := i.U
    threads(i).io.in.bits.rob_id    := 0.U(5.W)
    threads(i).io.in.bits.funct     := 0.U(8.W)
    threads(i).io.in.bits.waddr     := 0.U(14.W)
    threads(i).io.in.bits.mode      := 0.U(1.W)
  }

  when(io.in.valid){
    for (i <- 0 until 4){
      when (io.thread_id === i.U) {
        threads(i).io.in <> io.in
      }
    }
  }
  VecQueue(0).io.west       <> io.west
  VecQueue(0).io.north      <> threads(0).io.out
  io.in.ready := true.B
  for (i <- 1 until 4) {
    VecQueue(i).io.north <> threads(i).io.out
...
class Vec8PE extends Module{
  ...
  val threads = Seq.fill(8)(Module(new VecThread()))
  for(i <- 0 until 8){
    threads(i).io.in.valid          := false.B
    threads(i).io.in.bits.op1       := VecInit(Seq.fill(16)(0.U(8.W)))
    threads(i).io.in.bits.op2       := VecInit(Seq.fill(16)(0.U(8.W)))
