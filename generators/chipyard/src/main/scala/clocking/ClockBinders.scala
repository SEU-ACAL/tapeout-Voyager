...
class WithPLLSelectorDividerClockGenerator(enable: Boolean = true) extends OverrideLazyIOBinder({
  (system: HasChipyardPRCI) => {
    ...
    InModuleBody {
      val clock_wire_o = Wire(Output(Bool()))//PLL clock to output
      val reset_wire = Wire(Input(AsyncReset()))
      val clock_wire = Wire(Input(Clock()))
      val (clock_io, clockIOCell) = IOCell.generateIOFromSignal(clock_wire, "clock", p(IOCellKey))
      val (reset_io, resetIOCell) = IOCell.generateIOFromSignal(reset_wire, "reset", p(IOCellKey))

      val (clock_io_o, clockoIOCell) = IOCell.generateIOFromSignal(clock_wire_o, "clock_o", p(IOCellKey))
      slowClockSource.out.unzip._1.map { o =>
        o.clock := clock_wire
        o.reset := reset_wire
      }
      ...
      fake_pll.io.gate := pllCtrlSink.in(0)._1.gate
      fake_pll.io.clock := clock_wire
      clock_wire_o := fake_pll.io.clk.asBool
      pllClockSource.out.unzip._1.map { o =>
        o.clock := fake_pll.io.clk
        o.reset := reset_wire
      }

      (Seq(ClockPort(() => clock_io, 500),PLLCLKPort(() => clock_io_o), ResetPort(() => reset_io)), clockIOCell++clockoIOCell ++ resetIOCell)
    }
  }
})
