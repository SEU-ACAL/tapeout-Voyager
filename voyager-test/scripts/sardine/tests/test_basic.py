...
def test_verilator_hello(script_runner, caplog):
  ...
  result = script_runner(f"{script_dir}/run-verilator.sh", ["--config", "VoyagerVerilatorConfig", "hello"], timeout=1200)
...
def test_verilator_bb_mvin_mvout(script_runner, caplog):
  ...
  start_time = time.time()
  result = script_runner(f"{script_dir}/run-verilator.sh", ["--config", "VoyagerVerilatorConfig", "bb_mvin_mvout_multi"], timeout=6000)
  execution_time = time.time() - start_time
