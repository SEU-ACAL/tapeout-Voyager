...
def test_verilator_hello(script_runner, caplog):
  ...
  result = script_runner(f"{script_dir}/run-verilator.sh", ["--config", "VoyagerVerilatorConfig", "hello", "--debug"], timeout=1200)
...
#   start_time = time.time()
#   result = script_runner(f"{script_dir}/run-verilator.sh", ["--config", "VoyagerVerilatorConfig", "bb_mvin_mvout_multi", "--debug"], timeout=6000)
#   execution_time = time.time() - start_time
