...
*/
#include "coremark.h"
#include "../riscv64-baremetal/kprintf.h"
...
#error "Please define a way to initialize a memory block."
	...
	if (known_id>=0) {
		for (i=0 ; i<default_num_contexts; i++) {
			...
			if ((results[i].execs & ID_LIST) && 
				...
				ee_printf("[%u]ERROR! list crc 0x%04x - should be 0x%04x",i,results[i].crclist,list_known_crc[known_id]);
				kputs("");
				results[i].err++;
			}
			if ((results[i].execs & ID_MATRIX) &&
				(results[i].crcmatrix!=matrix_known_crc[known_id])) {
				ee_printf("[%u]ERROR! matrix crc 0x%04x - should be 0x%04x",i,results[i].crcmatrix,matrix_known_crc[known_id]);
				kputs("");
				results[i].err++;
			}
			if ((results[i].execs & ID_STATE) &&
				(results[i].crcstate!=state_known_crc[known_id])) {
				ee_printf("[%u]ERROR! state crc 0x%04x - should be 0x%04x",i,results[i].crcstate,state_known_crc[known_id]);
				kputs("");
				results[i].err++;
			}
	...
	if (time_in_secs(total_time) > 0) {
		ee_printf("Iterations/Sec   : %f",default_num_contexts*results[0].iterations/time_in_secs(total_time));
		kputs("");
	}
#else 
	...
	kputs("");
	if (time_in_secs(total_time) > 0) {
		ee_printf("Iterations/Sec   : %d",default_num_contexts*results[0].iterations/time_in_secs(total_time));
		kputs("");
	}
#endif
	...
	ee_printf("Iterations       : %lu", (long unsigned) default_num_contexts*results[0].iterations);
	kputs("");
